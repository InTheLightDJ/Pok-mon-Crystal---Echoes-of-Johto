class Player < Trainer
  ...
  class Pokedex
    ...
    def seen_form?(species, gender, shiny = nil, form)
      species_id = GameData::Species.try_get(species)&.species
      return false if species_id.nil?
      @seen_forms[species_id] ||= [[[], []], [[], []]]
      return false if !@seen_forms[species_id][gender]  # genderless (2) has no slot
      if shiny.nil?
    ...
    def set_last_form_seen(species, gender = 0, shiny = false, form = 0)
      @last_seen_forms[species] = [gender, shiny, form]
    end
    ...
    def register(species, gender = 0, shiny = false, form = 0, should_refresh_dexes = true)
      ...
      @last_seen_forms[species] ||= []
      @last_seen_forms[species] = [gender, shiny, form] if @last_seen_forms[species] == []
      self.refresh_accessible_dexes if should_refresh_dexes
    end
    ...
    def register_last_seen(pkmn)
      ...
      form = 0 if species_data.form_name.nil? || species_data.form_name.empty?
      @last_seen_forms[pkmn.species] = [pkmn.gender, pkmn.shiny?, form]
    end
