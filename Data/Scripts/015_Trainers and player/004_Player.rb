...
class Player < Trainer
  ...
  attr_reader   :coins
  # @return [Integer] the player's Bug Tokens
  attr_reader   :Bug_Tokens
  attr_accessor :bugshop_purchases
  ...
  # @return [Boolean] whether the player has faster shoes (run speed 4.5 instead of 4)
  attr_accessor :has_faster_shoes
  ...
  # @return [Integer] index of last used  Poké Ball in the bag
  attr_accessor :lastPokeballIndex
  ...
  # Sets the player's BugTokens amount. It can not exceed
  # {Settings::MAX_Bug_Tokens}.
  # @param value [Integer] new Battle Points value
  def Bug_Tokens=(value)
    validate value => Integer
    @Bug_Tokens = value.clamp(0, Settings::MAX_Bug_Tokens)
  end

  def bugshop_purchases
  @bugshop_purchases = {} if !@bugshop_purchases.is_a?(Hash)
  return @bugshop_purchases
end
