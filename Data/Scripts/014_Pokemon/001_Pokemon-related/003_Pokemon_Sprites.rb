...
class PokemonSprite < Sprite
  ...
  def setSpeciesBitmap(species, gender = 0, shiny = false, back = false, form = 0, shadow = false, egg = false)
    @_iconbitmap&.dispose
    @_iconbitmap = GameData::Species.sprite_bitmap(species, gender, shiny, back, form, shadow, egg)
    self.bitmap = (@_iconbitmap) ? @_iconbitmap.bitmap : nil
    changeOrigin
  end
...
class PokemonIconSprite < Sprite
  ...
  def pokemon=(value)
    ...
    hue  = value.recolor_hue.to_i
    gray = value.recolor_gray.to_i
    @animBitmap.bitmap.hue_change(hue) if hue > 0
...
class PokemonSpeciesIconSprite < Sprite
  ...
  def pbSetParams(species, gender, shiny = false, form)
    @species = species
    @gender  = gender
    @form    = form
    @shiny   = shiny
