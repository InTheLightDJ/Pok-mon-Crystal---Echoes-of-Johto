module GameData
  class Move
    ...
    attr_reader :contest_type    # Symbol (:COOL/:BEAUTY/:CUTE/:SMART/:TOUGH) or nil
    attr_reader :contest_appeal  # Hearts gained (0 = no contest effect)
    attr_reader :contest_jam     # Hearts stolen from next performer
    ...
    def initialize(hash)
      ...
      @contest_type   = hash[:contest_type] ? hash[:contest_type].upcase.to_sym : nil
      @contest_appeal = hash[:contest_appeal] || 0
      @contest_jam    = hash[:contest_jam]    || 0
    end

    # Used by the PBS Updater
    ...
    def has_contest_data?
      return !@contest_type.nil?
    end
