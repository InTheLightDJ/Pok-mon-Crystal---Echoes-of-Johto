...
def pbCaveEntrance
  puts "[EscapePoint] pbCaveEntrance called — map=#{$game_map.map_id} player=#{$game_player.x},#{$game_player.y} dir=#{$game_player.direction}"
  pbSetEscapePoint
  puts "[EscapePoint] After pbSetEscapePoint: #{$PokemonGlobal.escapePoint.inspect}"
  pbCaveEntranceEx(false)
end

def pbCaveExit
  puts "[EscapePoint] pbCaveExit called — current point: #{$PokemonGlobal.escapePoint.inspect}"
  pbEraseEscapePoint
