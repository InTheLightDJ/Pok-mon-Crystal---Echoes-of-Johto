...
def pbHiddenMoveAnimation(pokemon)
  ...
  clas_mode = false # if true, classic mode (window); strobes mode if false
  if clas_mode
    # Create a temporary window
    tempWindow = Window_AdvancedTextPokemon.new("")
    tempWindow.viewport = Viewport.new(0, 0, Graphics.width, Graphics.height)
    tempWindow.viewport.z = 99999
    # Window's size and position
    windowWidth = 136
    windowHeight = 136
    windowX = (Graphics.width - windowWidth) / 2
    windowY = (Graphics.height - windowHeight) / 2
    tempWindow.x = windowX
    tempWindow.y = windowY
    tempWindow.width = windowWidth
    tempWindow.height = windowHeight
    # Pokémon's sprite
    sprite = PokemonSprite.new(tempWindow.viewport)
    sprite.setPokemonBitmap(pokemon)
    # Difference between window's half and sprite's half
    width_diff = (windowWidth / 2) - (sprite.bitmap.width / 2)
    height_diff = (windowHeight / 2) - (sprite.bitmap.height / 2)
    # Position the sprite
    sprite.x = windowX + (windowWidth - sprite.bitmap.width) + (sprite.bitmap.width / 2) - width_diff
    sprite.y = windowY + (windowHeight - sprite.bitmap.height) + (sprite.bitmap.height / 2) - height_diff
    sprite.z = tempWindow.viewport.z + 1
    # Updates the screen to see the changes
    cry_duration = GameData::Species.cry_length(pokemon)
    pokemon.play_cry
    pbWait(cry_duration + 0.5) # Wait 1 second (60 frames)
    # End
    sprite.dispose
    tempWindow.dispose
  else
...
HiddenMoveHandlers::UseMove.add(:FLASH, proc { |move, pokemon|
  ...
  pbFadeOutIn do
    # Refresh the TilemapRenderer so isDark re-evaluates and tile color overlay clears.
...
def pbHeadbuttEffect(event = nil)
 ...
 event ||= Game_Event.new($game_map.map_id, RPG::Event.new(0, 0), $game_map)
...
EventHandlers.add(:on_player_interact, :Headbuttree,
  proc {
   terrain = $game_player.pbFacingTerrainTag
    if terrain.id == :Headbuttree
      dir = $game_player.direction
      tile = pbFacingTile(dir, $game_player)
      dummy_event = DummyEvent.new(tile[1], tile[2])
      pbHeadbutt(dummy_event)
    end
  }
)
...
HiddenMoveHandlers::CanUseMove.add(:WHIRLPOOL, proc { |move, pkmn, showmsg|
  next false unless $PokemonGlobal&.surfing
  tag = $game_player.pbFacingTerrainTag rescue nil
  next !!tag&.whirlpool
})

HiddenMoveHandlers::UseMove.add(:WHIRLPOOL, proc { |move, pkmn|
  ...
  if !pbHiddenMoveAnimation(pkmn)
    pbMessage(_INTL("{1} used {2}!", pkmn.name, GameData::Move.get(move).name))
  end
  pbSEPlay("Whirlwind") rescue nil

  # Open THIS whirlpool tile just long enough to take exactly one step into it
  $game_temp.open_whirlpool = [$game_map.map_id, fx, fy]

  next true
})

EventHandlers.add(:on_player_interact, :prompt_whirlpool, proc {
  fx, fy = $game_player.front_xy
  tag = $game_map.terrain_tag(fx, fy) rescue nil
  next unless tag&.whirlpool

  if !$PokemonGlobal.surfing
    pbMessage(_INTL("A fierce whirlpool is raging. You could cross it while surfing."))
    next
  end

  user = $player.party.find { |pk| pk && !pk.egg? && pk.hasMove?(:WHIRLPOOL) }
  if !user
    pbMessage(_INTL("A fierce whirlpool is raging. A Pokémon may be able to whirl it away."))
    next
  end

  if pbConfirmMessage(_INTL("A fierce whirlpool is raging! Use Whirlpool?"))
    HiddenMoveHandlers.triggerUseMove(:WHIRLPOOL, user)
  end
})
