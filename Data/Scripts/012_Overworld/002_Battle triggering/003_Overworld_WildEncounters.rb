...
def pbGenerateWildPokemon(species, level, isRoamer = false)
  ...
  # same species
  if $PokemonGlobal&.radioStation == (:in_the_light || :rocket_takeover) && $game_map&.metadata&.outdoor_map
    genwildpoke.shiny = true if rand(100) == 0   # 1 in 2 shiny chance
    
      # Give the special move: 1 in 100
    if rand(100) == 0 && !genwildpoke.hasMove?(:INTHELIGHT)
      if genwildpoke.numMoves < Pokemon::MAX_MOVES
        genwildpoke.learn_move(:INTHELIGHT)
      else
        # replace last move (or pick a random slot if you prefer)
        genwildpoke.moves[Pokemon::MAX_MOVES - 1] = Pokemon::Move.new(:INTHELIGHT)
      end
    end
  end
...
# Safety guard: allow_encounter? expects [species, level]
# --- SAFETY GUARD: never crash if a Pokemon object is passed into allow_encounter? ---
#This is for the pre battle screen pokemon that flys across the screen
class PokemonEncounters #intro_flying_species - search feature
  # Ruby 1.8 in RGSS: instance_methods returns strings, so guard accordingly
  unless instance_methods(false).include?("__allow_encounter_orig")
    alias __allow_encounter_orig allow_encounter?
  end

  def allow_encounter?(enc_data, repel_active = false)
    if enc_data.is_a?(Pokemon)
      # Optional: log once in console to find the culprit
      # echoln "[encounters] Coerced Pokemon -> [#{enc_data.species}, #{enc_data.level}] in allow_encounter?"
      enc_data = [enc_data.species, enc_data.level]
    end
    return __allow_encounter_orig(enc_data, repel_active)
  end
end