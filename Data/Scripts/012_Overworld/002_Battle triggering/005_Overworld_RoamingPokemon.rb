...
# Resets a roamer back to active roaming status.
# Call from an event Script box:
#   pbResetRoamer(:ENTEI)   -> resets just Entei
#   pbResetRoamer(:RAIKOU)  -> resets just Raikou
#   pbResetRoamer           -> resets all roamers
def pbResetRoamer(species = nil)
  $PokemonGlobal.roamPokemon   ||= []
  $PokemonGlobal.roamPosition  ||= {}
  Settings::ROAMING_SPECIES.each_with_index do |data, i|
    next if species && data[0] != species
    $PokemonGlobal.roamPokemon[i]       = nil
    $PokemonGlobal.roamPokemonCaught[i] = false
    $PokemonGlobal.roamPosition[i]      = nil   # random map on next step
  end
end

# Returns true if the roamer's switch condition is satisfied.
# cond can be:
#   nil/0                 -> always allowed
#   Integer  n (>0)       -> allowed if $game_switches[n] is true
#   Integer -n (<0)       -> allowed if $game_switches[n] is false
#   Array of integers     -> all entries must be satisfied (positive = ON, negative = OFF)
def pbRoamerConditionMet?(cond)
  return true if cond.nil? || cond == 0
  if cond.is_a?(Array)
    return cond.all? { |c| c > 0 ? $game_switches[c] : !$game_switches[-c] }
  end
  return (cond > 0) ? $game_switches[cond] : !$game_switches[-cond]
end
...
def pbRoamPokemonOne(idxRoamer)
 ...
 return unless pbRoamerConditionMet?(roamData[2])
...
# Returns true if any active, uncaught roamer is on the player's current map.
def pbAnyRoamerOnCurrentMap?
  return false if !$PokemonGlobal.roamPosition
  currentMapId  = $game_map.map_id
  currentRegion = pbGetCurrentRegion
  currentName   = $game_map.name
  Settings::ROAMING_SPECIES.each_with_index do |data, i|
    next if !GameData::Species.exists?(data[0])
    next unless pbRoamerConditionMet?(data[2])
    next if $PokemonGlobal.roamPokemon[i] == true
    roamerMap = $PokemonGlobal.roamPosition[i]
    next if !roamerMap
    if roamerMap == currentMapId
      return true
    else
      map_meta = GameData::MapMetadata.try_get(roamerMap)
      next if !map_meta || !map_meta.town_map_position ||
              map_meta.town_map_position[0] != currentRegion
      return true if pbGetMapNameFromId(roamerMap) == currentName
    end
  end
  return false
end
...
# Pokémon roam. Also play proximity music if a roamer shares the current map.
EventHandlers.add(:on_enter_map, :move_roaming_pokemon,
  proc { |old_map_id|
    # Get and compare map names
    mapInfos = pbLoadMapInfos
    same_map  = mapInfos && old_map_id > 0 && mapInfos[old_map_id] &&
                mapInfos[old_map_id].name && $game_map.name == mapInfos[old_map_id].name
    unless same_map
      pbRoamPokemon
      $PokemonGlobal.roamedAlready = false
    end
    # Roamer proximity music
    if Settings::ROAMER_MUSIC && pbAnyRoamerOnCurrentMap?
      cur = $game_system&.playing_bgm
      roamer_bgm = "Added song"
      pbBGMPlay(roamer_bgm) if !cur || cur.name != roamer_bgm
    end
  }
)
