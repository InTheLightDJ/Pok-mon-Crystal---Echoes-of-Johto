...
EventHandlers.add(:on_step_taken, :grass_rustling,
  proc { |event|
    ...
    event.each_occupied_tile do |x, y|
      ...
      # Morn-Day/Night handler
      day = Settings::GRASS_ANIMATION_ID
      night = Settings::GRASS_NIGHT_ANIMATION_ID
      grass_anm = PBDayNight.isNight? ? night : day
      spriteset&.addUserAnimation(grass_anm, x, y, true, 1)
    end
  }
)
...
def pbBattleOnStepTaken(repel_active)
  ...
  #for the pre battle flying intro pokemon 
  if $intro_flying_species && rand < 0.1  # 1% chance turns pokemon into screen transition pkmn 
    level = (encounter.respond_to?(:level) ? encounter.level : rand(10..20)) # Keep original level
    pkmn = Pokemon.new($intro_flying_species, level)
    pkmn.shiny = true if $intro_flying_species_shiny
    encounter = pkmn
  end
...
EventHandlers.add(:on_map_or_spriteset_change, :show_location_window,
  proc { |scene, map_changed|
    ...
    scene.spriteset.addUserSprite(LocationWindow.new(_INTL("<ac>{1}</ac>", $game_map.name))) if !nosignpost
  }
)
...
def pbSetEscapePoint
  ...
  puts "[EscapePoint] SET — map=#{$game_map.map_id} x=#{xco} y=#{yco} dir=#{dir}"
end

def pbEraseEscapePoint
  puts "[EscapePoint] ERASED (was: #{$PokemonGlobal.escapePoint.inspect})"
