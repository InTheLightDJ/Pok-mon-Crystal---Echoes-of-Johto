  ...
  def set_event_graphic(berry_plant, full_check = false)
    ...
    when 0
      ...
      if berry_plant.growth_stage == 1
        ...
        night = PBDayNight.isNight?
        if Settings:CUSTOM_BERRY_GRAPHICS
          if night
            filename = sprintf("berrytree_%s_nite", GameData::Item.get(berry_plant.berry_id).id.to_s)
          else
            filename = sprintf("berrytree_%s", GameData::Item.get(berry_plant.berry_id).id.to_s)
          end
        else
          if night
            filename = sprintf("berrytree_day")
          else
            filename = sprintf("berrytree_nite")
          end
        end
