...
class Interpreter
  ...
  def setSelfSwitchExt(start_id, end_id, switch_name, value, mapid = -1)
  mapid = @map_id || $game_map.map_id if mapid < 0
  (start_id..end_id).each do |event_id|
    old_value = $game_self_switches[[mapid, event_id, switch_name]]
     $game_self_switches[[mapid, event_id, switch_name]] = value
     if value != old_value && $map_factory.hasMap?(mapid)
      $map_factory.getMap(mapid, false).need_refresh = true
    end
  end
