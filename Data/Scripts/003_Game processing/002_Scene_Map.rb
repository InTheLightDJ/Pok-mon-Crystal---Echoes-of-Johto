...
# ----------------------------------------------------------------
# InTheLight flickering tone effect
# ----------------------------------------------------------------
_itl_active = ([:in_the_light, :rocket_takeover].include?($PokemonGlobal&.radioStation)) &&
              $game_map&.metadata&.outdoor_map
if _itl_active
  @in_the_light_flicker_timer ||= 0
  @in_the_light_flicker_timer += 1

  # Flicker every 10 frames (you can adjust this)
  if @in_the_light_flicker_timer % 10 == 0
    r = [-10, 0, 10].sample
    g = [-10, 0, 10].sample
    b = [10, 15, 20].sample
    gray = [5, 10, 15].sample
    $game_screen.start_tone_change(Tone.new(r, g, b, gray), 4)
  end
else
  if @in_the_light_flicker_timer && @in_the_light_flicker_timer > 0
    @in_the_light_flicker_timer = 0
