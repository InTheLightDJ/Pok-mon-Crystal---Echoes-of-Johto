    ...
    @sprites.each_with_index do |sprite, i|
      ...
      if first_pkmn.status == :NONE || !FollowingPkmn::APPLY_STATUS_TONES
        if $PokemonGlobal.follower_hold_item
          $game_temp.item_pulse ||= [50.0, 50.0, 150.0, (100 / (Graphics.frame_rate * 1.5))]
          $game_temp.item_pulse[0] += $game_temp.item_pulse[3]
          $game_temp.item_pulse[3] *= -1 if $game_temp.item_pulse[0] < $game_temp.item_pulse[1] ||
                                              $game_temp.item_pulse[0] > $game_temp.item_pulse[2]
          sprite.color.set(255, 255, 255, $game_temp.item_pulse[0])
        else
          sprite.color.set(0, 0, 0, 0)
          $game_temp.item_pulse = nil
        end
