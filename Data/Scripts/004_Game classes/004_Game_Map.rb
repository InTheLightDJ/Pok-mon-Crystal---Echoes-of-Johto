...
class Game_Map
  ...
  def setup(map_id)
    ...
    case @map.tileset_id
      when 1, 2, 3
        @terrain_tags[428] = 18  # Tile at row 5, column 4 (base tile index 384 + 44 = 428)
      end
    @fog_ox                  = 0
    @fog_oy                  = 0
  ...
  #===================================================================================
  #          GSC Time System: Swap tilesets depending on the time of the day.
  #
  #  Notes:
  #    1. Numbers indicate the tileset ID in the Database.
  #    2. MORN_TILESETS compile the data for morning tilesets.
  #         - It's active when switch PBDayNight.isMorning? is ON.
  #    3. EVE_TILESETS compile the data for night-tone tilesets.
  #         - It's active when switch PBDayNight.isNight? is ON.
  #    4. Since this system recreates the time mechanics of the Pokémon GBC games,
  #       it ONLY works if TIME_SHADING = false. If you want to use a timing system that 
  #       uses color tones, change the value to 'true'.
  #
  #===================================================================================
  MORN_TILESETS = {        # Tilesets change list for the morning
    1 => 2,
    4 => 5,
    40 => 42,
    41 => 42,
  ...
  EVE_TILESETS = {        # Tilesets change list for the night
    1 => 3,
    4 => 6,
    40 => 43,
    41 => 43,
  ...
  def getTilesetID(id)
    if !Settings::TIME_SHADING
      if(MORN_TILESETS.include?(id) && PBDayNight.isMorning?(pbGetTimeNow))
        return MORN_TILESETS[id]
      elsif (EVE_TILESETS.include?(id) && PBDayNight.isNight?(pbGetTimeNow))
        return EVE_TILESETS[id]
      else
        return id
      end
    else
  ...
  def pbChangeTileset
    tileset = $data_tilesets[getTilesetID(@map.tileset_id)]
    return if @tileset_name == tileset.tileset_name
  ...
  def update
    ...
    if pbGetTimeNow.hour - @last_update >= 1 || pbGetTimeNow.hour - @last_update <= -1
      @hour_now = pbGetTimeNow.hour
      if @last_update != @hour_now
        @need_refresh = true
        @last_update = pbGetTimeNow.hour
        pbChangeTileset
      end
