...
class Game_Player < Game_Character
  ...
  SCREEN_CENTER_X = ((Settings::SCREEN_WIDTH / 2) - 16 - (Game_Map::TILE_WIDTH / 2)) * Game_Map::X_SUBPIXELS
  ...
  def can_run?
    ...
    return false if !$player.has_running_shoes && !$player.has_faster_shoes &&
                    !($bag&.has?(:FASTERSHOES)) &&
                    !$PokemonGlobal.diving && !$PokemonGlobal.surfing && !$PokemonGlobal.bicycle
  ...
  def set_movement_type(type)
    ...
    when :running
      if !@move_route_forcing
        self.move_speed = ($player.has_faster_shoes || $bag&.has?(:FASTERSHOES)) ? 4.5 : 4
      end
