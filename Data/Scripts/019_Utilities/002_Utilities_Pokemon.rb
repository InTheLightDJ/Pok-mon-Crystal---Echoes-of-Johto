...
def pbGenerateRandomBabyEgg
	...
	babySpecies.concat([:CLEFFA]*6)
	babySpecies.concat([:IGGLYBUFF]*6)
	babySpecies.concat([:SMOOCHUM]*6)
	babySpecies.concat([:TYROGUE]*4)
  babySpecies.concat([:ELEKID]*4)
  babySpecies.concat([:MAGBY]*4)
	speciesIndex = rand(babySpecies.length)
	speciesSelection = babySpecies[speciesIndex]
	return pbGenerateEgg(speciesSelection)
end
def pbGenerateRandomBabyEgg2
	babySpecies = Array.new
	babySpecies.concat([:PICHU]*1)
	babySpecies.concat([:CLEFFA]*1)
	babySpecies.concat([:IGGLYBUFF]*1)
	babySpecies.concat([:SMOOCHUM]*1)
	babySpecies.concat([:TYROGUE]*1)
  babySpecies.concat([:ELEKID]*1)
  babySpecies.concat([:MAGBY]*1)
  babySpecies.concat([:BULBASAUR]*1)
  babySpecies.concat([:CHARMANDER]*1)
  babySpecies.concat([:SQUIRTLE]*1)
  babySpecies.concat([:CATERPIE]*3)
  babySpecies.concat([:WEEDLE]*3)
  babySpecies.concat([:ZUBAT]*2)
  babySpecies.concat([:DIGLETT]*2)
  babySpecies.concat([:MEOWTH]*2)
  babySpecies.concat([:ABRA]*1)
  babySpecies.concat([:MACHOP]*1)
  babySpecies.concat([:PONYTA]*1)
  babySpecies.concat([:SLOWPOKE]*1)
  babySpecies.concat([:MAGNEMITE]*1)
  babySpecies.concat([:FARFETCHD]*1)
  babySpecies.concat([:SEEL]*1)
  babySpecies.concat([:GRIMER]*1)
  babySpecies.concat([:CUBONE]*1)
  babySpecies.concat([:MRMIME]*1)
	speciesIndex = rand(babySpecies.length)
	speciesSelection = babySpecies[speciesIndex]
	return pbGenerateEgg(speciesSelection)
end
