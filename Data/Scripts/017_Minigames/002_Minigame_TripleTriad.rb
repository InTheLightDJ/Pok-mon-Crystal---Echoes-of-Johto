...
#===============================================================================
class TriadSquare
  attr_accessor :owner, :orig_owner, :card, :type
...
class TriadScene
  def pbStartScene(battle)
    ...
    pbDrawTextPositions(
      ...
      [[@battle.opponentName, 52, 2, :center, Color.new(0, 0, 0), nil],
       [@battle.playerName, Graphics.width - 52, 2, :center, Color.new(0, 0, 0), nil]]
    )
      ...
      if doRefresh
        ...
        @cardIndexes.length.times do |i|
          if i == cardIndex   # Card being placed
            ...
            @sprites["player#{@cardIndexes[i]}"].y = 50 + (boardY * 58)
  ...
  def pbUpdateScore
    ...
    pbDrawTextPositions(
      bitmap,
      [[_INTL("{1}-{2}", oppscore, playerscore), Graphics.width / 2, 2, :center, Color.new(0, 0, 0), nil]]
    )
  end
...
#         attack += attacker.bonus(defender)
  ...
  def pbStartScreen(opponentName, minLevel, maxLevel, rules = nil, oppdeck = nil, prize = nil)
    ...
    if oppdeck.is_a?(Array) && oppdeck.length == self.maxCards   # Preset
      ...
      while candidates.length < 200
        ...
        next unless pbResolveBitmap(GameData::Species.icon_filename(card, card_data.form))
  ...
  if prize
    ...
    when 4   # NEW: winner chooses from converted cards
      ...
      (@width * @height).times do |i|
        ...
        if sq.owner == 1 && sq.orig_owner == 2
          ...
          converted_counts[sid] += 1
        end
      end

      if converted_counts.empty?
        @scene.pbDisplayPaused(_INTL("You didn't convert any of {1}'s cards.", @opponentName))
      else
        opts = converted_counts.keys
        labels = opts.map do |sid|
          nm  = GameData::Species.get(sid).name
          cnt = converted_counts[sid]
          (cnt > 1) ? _INTL("{1} x{2}", nm, cnt) : nm
        end
        choice = pbMessage(_INTL("Choose your prize card."), labels, -1)  # -1 on cancel
        if choice && choice >= 0
          take_sid = opts[choice]
          if $PokemonGlobal.triads.add(take_sid)
            @scene.pbDisplayPaused(_INTL("Took {1} card.", GameData::Species.get(take_sid).name))
          else
            @scene.pbDisplayPaused(_INTL("You can't carry any more cards."))
          end
        end
      end
    end
...
# Card shop screen
#===============================================================================
def pbBuyTriads(discount = 0)
  commands = []
  realcommands = []
  GameData::Species.each_species do |s|
    next if !$player.owned?(s.species)
    price = TriadCard.new(s.id).price(discount)
  ...
  until done
    loop do
      ...
      if Input.trigger?(Input::USE)
        ...
        price = TriadCard.new(item).price(discount)
        if price == 0
          pbDisplayPaused(_INTL("The {1} card? Im not interested in that card right now.", itemname))
          break
        end
        cmdwindow.active = false
        cmdwindow.update
