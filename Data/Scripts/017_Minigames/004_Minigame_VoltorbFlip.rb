...
class VoltorbFlip
  ...
  def pbNewGame
    ...
    25.times do |i|
      ...
      @squares[i] = [((i % 5) * 40) + 82, ((i / 5).abs * 40) + TOP_Y, squareValues[r], false]
    ...
    # Draw current level
    pbDrawShadowText(@sprites["level"].bitmap, 280, 18, 118, 28,
                     _INTL("{1}", @level.to_s), Color.black, nil)
...
#      @quit = true
  ...
  def pbCreateSprites
    ...
    pbSetSmallFont(@sprites["numbers"].bitmap)
    ...
    3.times do |i|
      25.times do |j|
        ...
        icons[j] = [@directory + "tiles", @squares[j][0], @squares[j][1], 320 + (i * 34) + (points * 34), 0, 34, 34]
      end
      icons.compact!
    ...
    @cursor[0] = [@directory + "cursor", 82, TOP_Y, 0, 0, 34, 34]
  end
  ...
  def getInput
    if Input.trigger?(Input::UP)
      ...
      if @cursor[0][3] == 34   # If in mark mode
        @squares.length.times do |i|
          if (@index[0] * 40) + 82 == @squares[i][0] && (@index[1] * 40) + TOP_Y == @squares[i][1] && @squares[i][3] == false
            pbSEPlay("Voltorb Flip mark")
          end
        end
        (@marks.length + 1).times do |i|
          if @marks[i].nil?
            @marks[i] = [@directory + "tiles", (@index[0] * 40) + 82, (@index[1] * 40) + TOP_Y, 136, 0, 34, 34]
          elsif @marks[i][1] == (@index[0] * 40) + 82 && @marks[i][2] == (@index[1] * 40) + TOP_Y
            @marks.delete_at(i)
            @marks.compact!
        ...
        @squares.length.times do |i|
          if (@index[0] * 40) + 82 == @squares[i][0] && (@index[1] * 40) + TOP_Y == @squares[i][1] && @squares[i][3] == false
            pbAnimateTile((@index[0] * 40) + 82, (@index[1] * 40) + TOP_Y, @squares[i][2])
            @squares[i][3] = true
            ...
            if @squares[i][2] == 0
              ...
              animation = []
              3.times do |j|
                animation[0] = icons[0] = [@directory + "tiles", (@index[0] * 40) + 82, (@index[1] * 40) + TOP_Y,
                                           374 + (34 * j), 0, 34, 34]
              ...
              animation = []
              6.times do |j|
                animation[0] = [@directory + "explosion", (@index[0] * 40) - 14 + 82, (@index[1] * 40) - 14 + TOP_Y,
                                j * 64, 0, 64, 64]
                pbDrawImagePositions(@sprites["animation"].bitmap, animation)
                pbWait(0.1)
                @sprites["animation"].bitmap.clear
              ...
              pbDrawShadowText(@sprites["level"].bitmap, 280, 18, 118, 28,_INTL("{1}", @level.to_s), Color.black, nil)
              @points = 0
              pbUpdateCoins
              ...
              animation = []
              4.times do |j|
                animation[0] = [@directory + "flipAnimation", (@index[0] * 40) - 6 + 82, (@index[1] * 40) - 6 + TOP_Y,
                                j * 46, 0, 46, 48]
                pbDrawImagePositions(@sprites["animation"].bitmap, animation)
                pbWait(0.05)
                @sprites["animation"].bitmap.clear
...
#            pbMessage(_INTL("You can receive even more Coins in the next game!"))
      ...
      if @cursor[0][3] == 0 # If in normal mode
        @cursor[0] = [@directory + "cursor", 82, TOP_Y, 34, 0, 34, 34]
  ...
  def pbUpdateRowNumbers(num, voltorbs, i)
    ...
    numbers = [[numText, 316, TOP_Y + 2 + (i * 40), :right, Color.black]]
    voltorbNumbers = [[voltorbs.to_s, 316, TOP_Y + 18 + (i * 40), :right, Color.black]]
    pbDrawTextPositions(@sprites["numbers"].bitmap, numbers)
    pbDrawTextPositions(@sprites["numbers"].bitmap, voltorbNumbers)
  end

  def pbUpdateColumnNumbers(num, voltorbs, i)
    numText = sprintf("%02d", num)
    numbers = [[numText, 114 + (i * 40), TOP_Y + 204, :right, Color.black]]
    voltorbNumbers = [[voltorbs.to_s, 114 + (i * 40), TOP_Y + 220, :right, Color.black]]
    pbDrawTextPositions(@sprites["numbers"].bitmap, numbers)
    pbDrawTextPositions(@sprites["numbers"].bitmap, voltorbNumbers)
  end

  def pbCreateCoins(source, x)
    coinText = sprintf("%05d", source)
    coinText.chars.each_with_index do |digit, i|
      @coins[i] = [@directory + "numbers", x + (i * 16), 20, digit.to_i * 16, 0, 16, 12]
    end
  end
    ...
    5.times do |i|
      ...
      5.times do |j|
        icons[j] = [@directory + "tiles", @squares[i + (j * 5)][0], @squares[i + (j * 5)][1],
                    238 + (@squares[i + (j * 5)][2] * 34), 0, 34, 34]
      end
      pbDrawImagePositions(@sprites[i].bitmap, icons)
      pbWait(0.05)
      5.times do |j|
