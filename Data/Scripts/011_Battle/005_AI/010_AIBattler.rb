...
class Battle::AI::AIBattler
  ...
  def wants_status_problem?(new_status)
    ...
    if ability_active?
      case ability_id
      when :GUTS
        return true if ![:SLEEP, :FROZEN, :DROWSY, :FROSTBITE].include?(new_status) &&
    ...
    return true if [:SLEEP, :DROWSY].include?(new_status) && check_for_move { |m| m.usableWhenAsleep? }
  ...
  def wants_ability?(ability = :NONE)
    ...
    # Get the Gen 9 base ability rating
  ...
  def wants_item?(item)
    ...
    # Get the Gen 9 base item rating
