class Battle
  ...
  def pbCanShowCommands?(idxBattler)
    ...
    return false if battler.isCommander? # Skips Tatsugiri's commands while in Dondozo's mouth.
    return true
  end
    ...
    loop do
      ...
      loop do
        ...
        when -2   # Debug
          pbDebugMenu
          next
        when -3   # Quick Poké Ball Button
          ball = $bag.pockets[3][$player.lastPokeballIndex][0]
          if pbOpposingBattlerCount(idxBattler)==1# && ball > 0
              @scene.pbSelectBattler(-1)
              pbThrowPokeBall(idxBattler,ball)
              pbConsumeItemInBag(ball,idxBattler)
              battler = @battlers[idxBattler]
              pkmn    = battler.pokemon if battler
              commandsEnd = true 
              break
