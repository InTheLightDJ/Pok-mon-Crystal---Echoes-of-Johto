class Battle
  ...
  def pbStartBattleSendOut(sendOuts)
    ...
    if wildBattle?
      ...
      when 2
        pbDisplayPaused(_INTL("Wild {1} and {2}\nappeared!", foeParty[0].name,
                              foeParty[1].name))
      when 3
        pbDisplayPaused(_INTL("Wild {1}, {2} and {3}\nappeared!", foeParty[0].name,
                              foeParty[1].name, foeParty[2].name))
      end
    else   # Trainer battle
      case @opponent.length
      when 1
        pbDisplayPaused(_INTL("{1}\nwants to battle!", @opponent[0].full_name))
      when 2
        pbDisplayPaused(_INTL("{1} and {2}\nwant to battle!", @opponent[0].full_name,
    ...
    [1, 0].each do |side|
      ...
      trainers.each_with_index do |t, i|
        ...
        case sent.length
        when 1
          msg += _INTL("{1}\nsent out {2}!", t.full_name, @battlers[sent[0]].name)
        when 2
          msg += _INTL("{1}\nsent out\n{2} and {3}!", t.full_name,
                       @battlers[sent[0]].name, @battlers[sent[1]].name)
        when 3
          msg += _INTL("{1}\nsent out\n{2}, {3} and {4}!", t.full_name,
  ...
  def pbEndOfBattle
    ...
    when Outcome::WIN
      ...
      if trainerBattle?
        ...
        case @opponent.length
        when 1
          pbDisplayPaused(_INTL("{1}\nwas defeated!", @opponent[0].full_name))
        when 2
          pbDisplayPaused(_INTL("{1} and {2}\nwere defeated!", @opponent[0].full_name,
                                @opponent[1].full_name))
        when 3
          pbDisplayPaused(_INTL("{1}, {2} and {3\nwere defeated}!", @opponent[0].full_name,
