class Battle
  ...
  def pbCanSwitch?(idxBattler, idxParty = -1, partyScene = nil)
    ...
    # Ensure Pokemon affected by Commander cannot switch out for any reason.
    if @battlers[idxBattler].effects[PBEffects::Commander]
      partyScene&.pbDisplay(_INTL("{1} can't be switched out!", battler.pbThis))
      return false
    end
  ...
  def pbMessagesOnReplace(idxBattler, idxParty)
    ...
    if pbOwnedByPlayer?(idxBattler)
      ...
      pbDisplayBrief(_INTL("{1}\nsent out {2}!", owner.full_name, newPkmnName))
    end
  end
