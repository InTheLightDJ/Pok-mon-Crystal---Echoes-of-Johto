...
class Battle
  ...
  def pbThisEx(idxBattler, idxParty)
    party = pbParty(idxBattler)
    if opposes?(idxBattler)
      return _INTL("Enemy {1}", party[idxParty].name) if trainerBattle?
      return _INTL("Wild {1}", party[idxParty].name)
    end
    return _INTL("Ally {1}", party[idxParty].name) if !pbOwnedByPlayer?(idxBattler)
  ...
  def pbStartWeather(user, newWeather, fixedDuration = false, showAnim = true)
    ...
    when :Hail
      ...
      elsif Settings::HAIL_WEATHER_TYPE == 1
