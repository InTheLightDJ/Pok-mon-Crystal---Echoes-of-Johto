class Battle::Scene
  ...
  def pbBattleIntroAnimation
    ...
    if @battle.trainerBattle?
      ...
      @animations.push(Animation::Intro3.new(@sprites, @viewport, @battle)) # Player
      @animations.push(Animation::Intro4.new(@sprites, @viewport, @battle)) # Trainer
      pbWait(0.1)
      pbSEPlay("Positioned")
    ...
    # Set up wild Pokémon and Player returning to normal colour and playing intro
  ...
  def pbAnimation(moveID, user, targets, hitNum = 0)
    # GBC games battle feature - hides the user's databox until the animation completes
    ...
    # Shows again the user's databox
    sleep(0.5)
    ensure
    # Always re-show the databox
  ...
  def pbCommonAnimation(animName, user = nil, target = nil)
    ...
    # Commander - Keeps Tatsugiri's sprite hidden during Commander
