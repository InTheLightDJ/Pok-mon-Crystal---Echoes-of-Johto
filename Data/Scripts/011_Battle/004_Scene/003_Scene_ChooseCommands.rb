class Battle::Scene
  ...
  def pbCommandMenuEx(idxBattler, texts, mode = 0)
    ...
    cw.pbRefreshShortcut(@battle)
    pbSelectBattler(idxBattler)
    ret = -1
    # Added Quick Poké Ball Button function
    # Pages 0 : Fight menu, 1 : Status Battle, 2 : Pokeball Menu
    battleStatePage = 0
    loop do
      oldIndex = cw.index
      pbUpdate(cw)
      ...
      when 0 # Fight Menu
        ...
        # Status Battle
          ...
          pbPlayDecisionSE
          battleStatePage = 1
          cw.showbattleStatePage(battleStatePage)
        # Pokeball Menu
        elsif Input.trigger?(Input::JUMPDOWN) && $bag.pockets[3].length > 0 && 
              @battle.wildBattle? && mode < 3
          pbPlayDecisionSE
          battleStatePage = 2
          cw.showbattleStatePage(battleStatePage)
        end
      when 1 # Status Battle
        if Input.trigger?(Input::BACK)
          ...
          pbPlayDecisionSE
          cw.changeIndexBattler(-1)
        elsif Input.trigger?(Input::RIGHT)
          pbPlayDecisionSE
          cw.changeIndexBattler(1)
        end
        cw.updateBattleStateDescription
      when 2 # Pokeball Menu
        if Input.trigger?(Input::BACK)
          pbPlayCancelSE
          battleStatePage = 0
          cw.showbattleStatePage(battleStatePage)
        elsif Input.trigger?(Input::USE) # Throw Poké Ball
          pbPlayDecisionSE
          cw.resetBattleStateScreen
          ret = -3
          break
        elsif Input.trigger?(Input::LEFT)
          pbPlayDecisionSE
          cw.changeIndexPokeball(-1)
        elsif Input.trigger?(Input::RIGHT)
          pbPlayDecisionSE
          cw.changeIndexPokeball(1)
        end
  ...
  def pbFightMenu(idxBattler, megaEvoPossible = false)
    ...
    for i in 0...Pokemon::MAX_MOVES
      countMaxMove += 1 if battler.moves[i]
    end
    loop do
  ...
  def pbItemMenu(idxBattler, _firstAction)
    ...
    loop do
      ...
      commands[commands.length]          = _INTL("Cancel")
      command = itemScene.pbShowCommands(nil, commands)
      ...
      when 1, 2, 3   # Use on Pokémon/Pokémon's move/battler
        ...
        pkmnScreen.pbStartScene(_INTL("Use on which κΜ?"), @battle.pbNumPositions(0, 0))
        idxParty = -1
        # Loop while in party screen
        loop do
          # Select a Pokémon
          pkmnScene.pbSetHelpText(_INTL("Use on which κΜ?"))
