...
module Battle::Scene::Animation::BallAnimationMixin
  ...
  def addBallSprite(ballX, ballY, poke_ball)
    ...
    if @ballSprite.bitmap.width > @ballSprite.bitmap.height
      @ballSprite.src_rect.width = @ballSprite.bitmap.height
      ball.setSrcSize(0, @ballSprite.bitmap.height, @ballSprite.bitmap.height)
    end
    return ball
  end
  ...
  def trainerThrowingFrames(ball, trainer, traSprite)
    ...
    if traSprite.bitmap.width > size
  ...
  def ballSetOpen(ball, delay, poke_ball)
    ...
    if @ballSprite && @ballSprite.bitmap.width > @ballSprite.bitmap.height
      ball.setSrcSize(delay, @ballSprite.bitmap.height, @ballSprite.bitmap.height)
    end
  end
  ...
  def battlerAppear(battler, delay, battlerX, battlerY, batSprite, color)
    ...
    battler.setXY(delay, battlerX, battlerY)
    battler.setZoom(delay + 2, 25)
    battler.setZoom(delay + 4, 50)
    battler.setZoom(delay + 6, 100, [batSprite, :pbPlayIntroAnimation])
    ...
    color.alpha = 0
    battler.setColor(delay + 6, color)
  end

  def battlerAbsorb(battler, delay, battlerX, battlerY, color)
    battler.setColor(delay, color)    # Change color of battler to a solid shade
    battler.setZoom(delay, 100)       # Shrink battler into Poké Ball
