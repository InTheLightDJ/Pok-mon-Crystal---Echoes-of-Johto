...
class Battle::Scene::Animation::Intro < Battle::Scene::Animation
  ...
  def createProcesses
    ...
    # Bases
    makeSlideSprite("base_0", 1, appearTime, PictureOrigin::BOTTOM) if @sprites["base_0"]
    makeSlideSprite("base_1", -1, appearTime, PictureOrigin::CENTER) if @sprites["base_1"]
...
# animations
#===============================================================================
class Battle::Scene::Animation::Intro3 < Battle::Scene::Animation
  def initialize(sprites, viewport, battle)
    @battle = battle
    super(sprites, viewport)
  end
...
#===============================================================================
# Shows Trainer sprite fading back to their normal color, and triggers their intro
# animations
#===============================================================================
class Battle::Scene::Animation::Intro4 < Battle::Scene::Animation
  def initialize(sprites, viewport, battle)
    @battle = battle
    super(sprites, viewport)
  end
...
class Battle::Scene::Animation::LineupAppear < Battle::Scene::Animation
  ...
  def resetGraphics(sprites)
    ...
    when 1   # Opposing lineup
      barX  = 18
      barY  = 30
      ballX = barX + 112   # 30 is width of ball icon
      ballY = barY + 4
    end
...
class Battle::Scene::Animation::PokeballPlayerSendOut < Battle::Scene::Animation
  ...
  def createProcesses
    ...
    # Battler animation
    battlerAppear(battler, delay + 2, battlerEndX, battlerEndY, batSprite, col)
    if @shadowVisible
...
class Battle::Scene::Animation::PokeballTrainerSendOut < Battle::Scene::Animation
  ...
  def createProcesses
    ...
    battler.setSE(delay, "Battle jump to ball")
    # Battler animation
    battlerAppear(battler, delay + 8, battlerEndX, battlerEndY, batSprite, col)
    if @shadowVisible
      # Set up shadow sprite
      shadow = addSprite(shaSprite, PictureOrigin::CENTER)
      shadow.setOpacity(0, 0)
...
class Battle::Scene::Animation::PokeballThrowCapture < Battle::Scene::Animation
  ...
  def createProcesses
    ...
    ballMidY   = 24
    ballEndX   = ballPos[0]
    ballEndY   = ballPos[1] - 14
    ballGroundY = ballPos[1] - 14
    ...
    ballOpenUp(ball, delay, @poke_ball, false, true)
    # Ball out anim
    ...
    #       top of it.
    battler.setSE(delay, "Battle jump to ball")
    battler.setZoom(delay + 2, 50)
    battler.setZoom(delay + 4, 25)
    battler.setZoom(delay + 6, 0)
    ...
    # Poké Ball jump
    createBallTrajectory(ball, delay + 5, 10,
                         ballEndX, ballEndY, ballEndX, ballEndY - 20, ballEndX, ballEndY)
    createBallTrajectory(ball, delay + 15, 5,
                         ballEndX, ballEndY, ballEndX, ballEndY - 5, ballEndX, ballEndY)
    # Poké Ball critical capture animation
    delay = ball.totalDuration + 20
    spritesheet_ball = (@tempSprites[@ballSpriteIndex] && @tempSprites[@ballSpriteIndex].width >= @tempSprites[@ballSpriteIndex].height)
    if @critCapture
      ball.setSE(delay, "Battle ball shake")
      ball.setSE(delay + 4, "Battle ball shake")
      if spritesheet_ball
        ball.setSrc(delay, 40, 0)
        ball.setSrc(delay + 1, 80, 0)
        ball.setSrc(delay + 3, 40, 0)
        #
        ball.setSrc(delay + 5, 80, 0)
        ball.setSrc(delay + 7, 0, 0)
      else
    ...
    [@numShakes, 3].min.times do |i|
      ...
      if spritesheet_ball
        ball.setSrc(delay, 40, 0)
        ball.setSrc(delay + 2, 80, 0)
        ball.setSrc(delay + 6, 0, 0)
      else
    ...
    if @success
      ...
      # Smoke Burst
      smoke.setSrcSize(delay, 80, 80)
      smoke.setSrc(delay, 0, 0)
      smoke.setVisible(delay, true)
      smoke.setZ(delay, batSprite.z + 1)
      smoke.setSrc(delay + 2, size, 0)
      smoke.setSrc(delay + 4, size * 2, 0)
      smoke.setVisible(delay + 8, false)
      # Battler emerges
...
class Battle::Scene::Animation::PokeballThrowDeflect < Battle::Scene::Animation
  ...
  def createProcesses
    ...
    createBallTrajectory(ball, 0, 16,
                         ballStartX, ballStartY, ballMidX, ballMidY, ballEndX, ballEndY)
    # Set up hit sprite
    hit = addNewSprite(ballEndX, ballEndY,
                         "Graphics/Battle animations/hit", PictureOrigin::CENTER)
    hit.setVisible(0, false)
    hit.setZ(0, 89)
    # Poké Ball knocked back
    delay = ball.totalDuration
    ball.setSE(delay, "Battle ball drop")
    ball.moveXY(delay, 8, 134, Graphics.height + 32)   # Back to player's corner
    # Show hit sprite appearing
    hit.setVisible(delay-2, true)
    hit.setVisible(delay+2, false)
  end
end