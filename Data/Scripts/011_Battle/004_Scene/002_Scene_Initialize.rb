class Battle::Scene
  ...
  def pbInitSprites
    ...
    2.times do |side|
      partyBar = pbAddSprite("partyBar_#{side}", 0, 0,
                             "Graphics/UI/Battle/overlay_teampreview", @viewport)
      partyBar.z       = 120
      partyBar.mirror  = true if side == 1   # Opponent's lineup bar only
  ...
  def pbCreateBackdropSprites
    ...
    battleBG   = "Graphics/Battlebacks/general_bg" if !pbResolveBitmap(battleBG)
    ...
    bg.mirror = true
    2.times do |side|
      next if !pbResolveBitmap("Graphics/Battlebacks/" + baseFilename + "_base#{side}")
