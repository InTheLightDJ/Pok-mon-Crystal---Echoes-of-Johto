...
class Battle::FakeBattler
  ...
  def pbThis(lowerCase = false)
    return (lowerCase) ? _INTL("wild {1}", name) : _INTL("Wild {1}", name)
  end
...
class Battle::Scene::SafariDataBox < Sprite
  ...
  def refresh
    ...
    textpos = []
    textpos.push([_INTL("Safari Balls"), 30, 8, :left, base, shadow])
    textpos.push([_INTL("Left: {1}", @battle.ballCount), 30, 38, :left, base, shadow])
...
class Battle::Scene::Animation::ThrowRock < Battle::Scene::Animation
  ...
  def createProcesses
    ...
    batSprite = @sprites["pokemon_#{@battler.index}"]
    traSprite = @sprites["player_1"]
    ballStartX = 124 # traSprite.x
    ballStartY = 180 # traSprite.y - (traSprite.bitmap.height / 2)
    ballMidX   = 0   # Unused in arc calculation
    ballMidY   = 48  # 122
    ...
    hit.setVisible(delay, true)
    hit.setVisible(delay + 4, false)
    # Set up anger sprite
    ...
    2.times do
      ...
      anger.setZoom(delay + 3, 130)
      anger.setZoom(delay + 6, 100)
  ...
  def pbSetSeen(battler)
    ...
    if battler.is_a?(Battle::Battler)
      pbPlayer.pokedex.register(battler.displaySpecies, battler.displayGender,
                                battler.shiny?, battler.displayForm)
    else
