class Battle::Battler
  ...
  def pbCanInflictStatus?(newStatus, user, showMessages, move = nil, ignoreStatus = false)
    ...
    # Replace status to drowsy or frostbite
    case newStatus
    ...
    if affectedByTerrain?
      ...
      when :Electric
        if [:SLEEP,:DROWSY].include?(newStatus)
          if showMessages
    ...
    # Uproar immunity
    if [:SLEEP,:DROWSY].include?(newStatus) && !(hasActiveAbility?(:SOUNDPROOF) && !@battle.moldBreaker)
      @battle.allBattlers.each do |b|
        next if b.effects[PBEffects::Uproar] == 0
        @battle.pbDisplay(_INTL("But the uproar kept {1} awake!", pbThis(true))) if showMessages && newStatus == :SLEEP
        @battle.pbDisplay(_INTL("But the uproar kept {1} alert!", pbThis(true))) if showMessages && newStatus == :DROWSY
        return false
      end
    end
    # Type immunities
    ...
    if immuneByAbility
      if showMessages
        ...
        if Battle::Scene::USE_ABILITY_SPLASH
          ...
          when :DROWSY    then msg = _INTL("{1} stays alert!", pbThis)
          ...
          when :DROWSY
            msg = _INTL("{1} stays alert because of {2}'s {3}!",
                        pbThis, immAlly.pbThis(true), immAlly.abilityName)
          when :POISON
  ...
  def pbCanSynchronizeStatus?(newStatus, user)
    ...
    # Replace newStatus to the prior status
    case newStatus
  ...
  def pbInflictStatus(newStatus, newStatusCount = 0, msg = nil, user = nil)
    ...
    if msg && !msg.empty?
      ...
      when :DROWSY
        @battle.pbDisplay(_INTL("{1} grew drowsy!\nIt may be too sleepy to move!", pbThis))
      when :POISON
        if newStatusCount > 0
