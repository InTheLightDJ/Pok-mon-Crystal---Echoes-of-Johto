...
ItemHandlers::UseFromBag.add(:ESCAPEROPE, proc { |item|
  puts "[EscapeRope] Used — escapePoint=#{($PokemonGlobal.escapePoint rescue 'ERROR').inspect} map=#{$game_map.map_id}"
...
ItemHandlers::UseFromBag.addIf(:move_machines,
 ...
 ItemHandlers::UseFromBag.add(:BOOSTERPACK, proc { |item|
   BoosterPack.give(item, 3, 0, BoosterPack::SHINY_ODDS_NORMAL)
   next 1
 })

 ItemHandlers::UseFromBag.add(:LEGENDPACK, proc { |item|
   BoosterPack.give(item, 3, 1, BoosterPack::SHINY_ODDS_LEGEND)
   next 1
 })
...
ItemHandlers::UseInField.add(:DAYSKIPBERRY, proc { |item|
  choice = pbMessage(_INTL("What do you want to do with the Day Skip Berry?"),
    [_INTL("Skip 24 hours"), _INTL("Next day at 4 AM"), _INTL("Cancel")], 3)

  case choice
  when 0  # Skip 24 hours (adds 86400 seconds)
    pbFadeOutIn {
      UnrealTime.add_seconds(86400)
      PBDayNight.sheduleToneRefresh
    }
    pbMessage(_INTL("Time skipped ahead by one day."))
    next 1
  when 1  # Set to 4 AM next day
    pbFadeOutIn {
      UnrealTime.advance_to(4, 0, 0)  # Advances to 4:00 AM, next day if past 4:00 AM
      PBDayNight.sheduleToneRefresh
    }
    pbMessage(_INTL("Time set to 4:00 AM the next day."))
    next 1
  end
...
ItemHandlers::UseOnPokemon.addIf(:pokeblocks,
  proc { |item| GameData::Item.get(item).has_flag?("Pokeblock") },
  proc { |item, qty, pkmn, scene|
    next false if pkmn.egg?
    if pkmn.sheen >= 255
      scene.pbDisplay(_INTL("{1} is full and can't eat any more!", pkmn.name))
      next false
    end
    item_data = GameData::Item.get(item)
    stat   = nil
    ...
    item_data.flags.each do |flag|
      if flag =~ /\APokeblock_([A-Za-z]+)_(\d+)\z/
        stat   = $~[1].downcase.to_sym
        amount = $~[2].to_i
        break
      end
    end
    next false if !stat || amount == 0
    old_val = pkmn.send(stat).to_i
    new_val = [old_val + amount, 255].min
    pkmn.send(:"#{stat}=", new_val)
    pkmn.sheen = [pkmn.sheen + (amount / 2), 255].min
    gained = new_val - old_val
    if gained > 0
      scene.pbDisplay(_INTL("{1}'s {2} condition rose by {3}!", pkmn.name, stat.to_s.capitalize, gained))
    else
      scene.pbDisplay(_INTL("{1}'s {2} condition is already maxed!", pkmn.name, stat.to_s.capitalize))
    end
    next gained > 0
  }
)
