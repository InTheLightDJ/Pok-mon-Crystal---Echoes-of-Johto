...
def pbChangeLevel(pkmn, new_level, scene)
  ...
  if old_level > new_level
    ...
    if scene.is_a?(PokemonPartyScreen)
      scene.pbDisplay(_INTL("{1} grew to Λ{2}!", pkmn.name, pkmn.level))
    else
      pbMessage(_INTL("{1} grew to Λ{2}!", pkmn.name, pkmn.level))
    end
    total_hp_diff        = pkmn.totalhp - old_total_hp
    attack_diff          = pkmn.attack - old_attack
    defense_diff         = pkmn.defense - old_defense
...
def pbNatureChangingMint(new_nature, item, pkmn, scene)
  ...
  scene.pbDisplay(_INTL("It might affect {1}'s stats.", pkmn.name))
  if !scene.pbConfirm(_INTL("Are you sure you want to use it?"))
    return false
  end
