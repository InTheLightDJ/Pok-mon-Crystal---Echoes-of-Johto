...
class Phone
  def self.get(*args)
  return $PokemonGlobal.phone.get(*args)
end
  module Call
    module_function
    ...
    def make_incoming
      ...
      return if !contact
      pbMessageCallingIn(contact.name)
    ...
    def start_message(contact = nil)
      return if $game_temp.calldataCaruban # return if its an incoming call
      pbSEPlay("Call", 80, 100)
      pbMessage("......\\wt[5] ......\1")
    end
...
def pbMessageCallingIn(caller = "")
  ...
  sprites["callbg"].setBitmap("Graphics/UI/Pokegear/phone_bg")
  sprites["call"] = IconSprite.new(0, 0, viewport)
  sprites["call"].setBitmap("Graphics/UI/Pokegear/phone_icon")
  sprites["overlay"] = BitmapSprite.new(Graphics.width, Graphics.height, viewport)
  sprites["overlay"].bitmap.clear
  baseColor   = Color.new(0, 0, 0)
  shadowColor = Color.new(248, 248, 248, 0)
  textPositions = [
    [_INTL("{1}:", caller), 48, 16, :left, baseColor, shadowColor],
  ]
  ...
  2.times do
    time = System.uptime
    pbSEPlay("Call")
    until System.uptime-time >= 1.5 do
      Graphics.update
      Input.update
      pbUpdateSceneMap
      if frame % 25 == 0
  ...
  $game_temp.calldataCaruban = ret
end

def pbMessageCallEnd
  data = $game_temp.calldataCaruban
  return if !data
  sprites = data[0]
  viewport = data[1]
  pbDisposeSpriteHash(sprites)
  viewport.dispose
  $game_temp.calldataCaruban = nil
end