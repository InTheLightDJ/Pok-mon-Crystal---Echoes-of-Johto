...
class SpriteWindow_DebugRoamers < Window_DrawableCommand
  ...
  def shadowtext(t, x, y, w, h, align = 0, colors = 0)
    ...
    pbDrawShadowText(self.contents, x, y, [width, w].max, h, t, base, nil)
  end
  ...
  def drawItem(index, _count, rect)
    ...
    status2Width = rect.width * 75 / 100
    pos = 1
    if index == self.itemCount - 2
      # Advance roaming
      self.shadowtext(_INTL("[Reset all roamer loc.]"), rect.x, rect.y, nameWidth, rect.height)
    elsif index == self.itemCount - 1
      # Advance roaming
      self.shadowtext(_INTL("[Clear all roamer loc.]"), rect.x, rect.y, nameWidth, rect.height)
    else
      pkmn = Settings::ROAMING_SPECIES[index]
      name = GameData::Species.get(pkmn[0]).name + " Λ#{pkmn[1]}"
      status = ""
      status2 = ""
      statuscolor = 0
      if pkmn[2] <= 0 || $game_switches[pkmn[2]]
        ...
        if status == true
          ...
          if curmap
            ...
            status = "[ROAM]"
            status2 = "[#{curmap}: #{mapinfos[curmap].name}]"
            text = mapinfos[curmap].name
            # Split text for the running text
            real_width = status2Width - 32#16
            if self.contents.text_size(status2).width > status2Width - 16 # Check if this text is too long for the window
              pos = 0
      ...
      self.shadowtext(status2, pos == 0 ? rect.x + (nameWidth * 1 / 2).ceil : rect.width, rect.y+16, status2Width, rect.height, pos, statuscolor)
      self.shadowtext("]", rect.width, rect.y+16, rect.width, rect.height, 0, statuscolor) if pos == 0
    end
  end
