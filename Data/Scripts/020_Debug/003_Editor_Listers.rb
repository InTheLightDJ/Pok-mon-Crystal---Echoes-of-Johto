...
# Core lister script
#===============================================================================
def pbListWindow(cmds, width = Graphics.width)
  list = Window_CommandPokemon.newWithSize(cmds, 0, Graphics.height / 2 + 32, width, Graphics.height / 2 - 32)
  list.index     = 0
  list.rowHeight = 24
...
def pbListScreen(title, lister)
  ...
  title = Window_UnformattedTextPokemon.newWithSize(
    title, 0, 0, Graphics.width, 54, viewport
  )
  title.z = 2
...
def pbListScreenBlock(title, lister)
  viewport = Viewport.new(0, 0, Graphics.width, Graphics.height)
  viewport.z = 99999
  list = pbListWindow([], Graphics.width)
  list.viewport = viewport
  list.z        = 2
  title = Window_UnformattedTextPokemon.newWithSize(
    title, 0, 0, Graphics.width, 54, viewport
...
class ItemLister
  def initialize(selection = 0, includeNew = false)
    @sprite = ItemIconSprite.new(Graphics.width / 2, Graphics.height / 2, nil)
    @sprite.z = 2
    @selection = selection
    @commands = []
    @ids = []
...
class TrainerTypeLister
  def initialize(selection = 0, includeNew = false)
    @sprite = IconSprite.new(Graphics.width / 2, (Graphics.height / 2) - 32)
    @sprite.z = 2
    @selection = selection
    @commands = []
    @ids = []
...
class TrainerBattleLister
  def initialize(selection, includeNew)
    @sprite = IconSprite.new((Graphics.width / 2) - 84, (Graphics.height / 2) + 32)
    @sprite.z = 2
    @pkmnList = Window_UnformattedTextPokemon.newWithSize(
      "", Graphics.width / 2, Graphics.height / 2 - 90, Graphics.width / 2 + 8, 64
    )
    pbSetSmallFont(@pkmnList.contents)
    @pkmnList.z = 3
    @selection = selection
    @commands = []
    @ids = []
    ...
    if !@includeNew || index > 0
      ...
      if tr_data
        tr_data.pokemon.each_with_index do |pkmn, i|
          text += "\n" if i > 0
          text += sprintf("%s(%d)", GameData::Species.get(pkmn[:species]).real_name, pkmn[:level])
        end
      end
    end
    @pkmnList.text = text
