...
class PokemonTilesetScene
  ...
  def initialize
    ...
    @sprites["title"].height   = 54
    @sprites["title"].x        = 0
    @sprites["title"].y        = 0
    @sprites["tileset"] = BitmapSprite.new(TILESET_WIDTH, Graphics.height - @sprites["title"].height - 104, @viewport)
    @sprites["tileset"].x = 32
    @sprites["tileset"].y = @sprites["title"].height + 104
    @sprites["tileset"].z = 10
    @sprites["overlay"] = BitmapSprite.new(Graphics.width, Graphics.height - @sprites["title"].height - 104, @viewport)
    @sprites["overlay"].x = 32
    @sprites["overlay"].y = @sprites["title"].height + 104
  ...
  def draw_overlay
    ...
    @visible_height.times do |yy|
      TILES_PER_ROW.times do |xx|
        ...
        next if terr == 0
        textpos.push([terr.to_s, (xx * TILE_SIZE) + (TILE_SIZE / 2) + 1, (yy * TILE_SIZE) + 8,
  ...
  def draw_tile_details
    ...
    overlay.clear
    tile_size = 2   # Size multiplier
    tile_x = (Graphics.width / 2) - TILE_SIZE
    tile_y = @sprites["title"].height + (TILE_SIZE / 2)
    ...
    textpos = [
      [_INTL("Terrain Tag: {1}", terrain_tag_name), (Graphics.width / 2), tile_y * 2, :center, Color.white, Color.black]
    ]
    # Draw all text
