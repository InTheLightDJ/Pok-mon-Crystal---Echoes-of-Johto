...
def pbEncountersEditor
  ...
  commands = []
  maps = []
  list = Window_CommandPokemon.newWithSize([], 0, 54, Graphics.width, Graphics.height - 54)
  help_window = Window_UnformattedTextPokemon.newWithSize(
    _INTL("Edit wild encounters"), 0, 240, Graphics.width, 54
  )
  help_window.z = 99999
  ret = 0
  need_refresh = true
...
def pbEncounterMapVersionEditor(enc_data)
 ...
 list = Window_CommandPokemon.newWithSize([], 0, 54, Graphics.width, Graphics.height - 54)
  help_window = Window_UnformattedTextPokemon.newWithSize(
    _INTL("Edit map's encounters"), 0, 240, Graphics.width, 54
  )
  help_window.z = 99999
  ret = 0
  need_refresh = true
...
def pbEncounterTypeEditor(enc_data, enc_type)
  ...
  list = Window_CommandPokemon.newWithSize([], 0, 54, Graphics.width, Graphics.height - 54)
  help_window = Window_UnformattedTextPokemon.newWithSize(
    _INTL("Edit encounter slots"), 0, 240, Graphics.width, 54
  )
  help_window.z = 99999
  enc_type_name = ""
  ret = 0
...
module TrainerBattleProperty
  ...
  def self.set(settingname, oldsetting)
    ...
    properties = [
      [_INTL("Trainer Type"), TrainerTypeProperty,     _INTL("Trainer type's name.")],
      [_INTL("Trainer Name"), StringProperty,          _INTL("Trainer's name.")],
      [_INTL("Version"),      LimitProperty.new(9999), _INTL("Version number.")],
      [_INTL("Lose Text"),    StringProperty,          _INTL("Shown in battle when is defeated.")]
    ]
    Settings::MAX_PARTY_SIZE.times do |i|
      properties.push([_INTL("Pokémon {1}", i + 1), TrainerPokemonProperty, _INTL("A Pokémon owned by the Trainer.")])
    end
    NUM_ITEMS.times do |i|
      properties.push([_INTL("Item {1}", i + 1), ItemProperty, _INTL("An item used by the Trainer.")])
    end
