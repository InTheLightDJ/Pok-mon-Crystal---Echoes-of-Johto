...
class Window_AdvancedTextPokemon < SpriteWindow_Base
    ...
    if value.length == 0
      ...
      if @letterbyletter
        ...
        fmt = getFormattedText(self.contents, 0, 0,
                               self.width - self.borderX - SpriteWindow_Base::TEXT_PADDING + 20, -1,
        ...
        @fmtchars = getFormattedText(self.contents, 0, 0,
                                     self.width - self.borderX - SpriteWindow_Base::TEXT_PADDING + 8, -1,
                                     shadowc3tag(@baseColor, @shadowColor) + value, @lineHeight, true)
        @oldfont = self.contents.font.clone
        @fmtchars.each do |ch|
          chx = ch[1] + ch[3]
  ...
  def allocPause
    ...
    @pausesprite = AnimatedSprite.create("Graphics/UI/pause_arrow", 2, 12)
...
module UpDownArrowMixin
  def initUpDownArrow
    @uparrow   = AnimatedSprite.create("Graphics/UI/arrow_up", 8, 2, self.viewport)
    @downarrow = AnimatedSprite.create("Graphics/UI/arrow_down", 8, 2, self.viewport)
    RPG::Cache.retain("Graphics/UI/arrow_up")
    RPG::Cache.retain("Graphics/UI/arrow_down")
    @uparrow.z   = 99998
    @downarrow.z = 99998
...
class Window_CommandPokemon < Window_DrawableCommand
  ...
  def update
    ...
    if old_index == self.index && @text_move_timer
      if System.uptime - @text_move_timer > 1 && @text_too_long && @text_phase == 0 # Wait at the start of text
        @text_shift += 1
        @text_move_timer = System.uptime
        @text_phase = 1
        refresh
      elsif System.uptime - @text_move_timer > 0.2 && @text_too_long && @text_phase == 1 # Running Text
        @text_shift += 1
        @text_move_timer = System.uptime
        refresh
      elsif System.uptime - @text_move_timer > 1 && @text_too_long && @text_phase == 2 # Wait at the end of text
        @text_shift = 0
        @text_phase = 0
        @text_move_timer = System.uptime
        refresh
      end
    # Reset the running text
    elsif old_index != self.index
      @text_move_timer = System.uptime
      @text_phase = 0
      @text_shift = 0
    end
...
class Window_AdvancedCommandPokemon < Window_DrawableCommand
  ...
  def drawItem(index, _count, rect)
    ...
    if toUnformattedText(@commands[index]).gsub(/\n/, "") == @commands[index]
      ...
      pbDrawShadowText(self.contents, rect.x, rect.y + (self.contents.text_offset_y || 0),
                       rect.width, rect.height, splitText(rect, index, @commands[index]), self.baseColor, self.shadowColor)
    else
      ...
      drawFormattedChars(self.contents, splitTextChar(rect, index, chars))
    end
  end
  ...
  def splitTextChar(rect, index, chars)
    # chars
    # letter, x, y, width, height
    # Split text for the running text
    real_width = rect.width - 16
    text = ""
    chars.each{|char| text += char[0] }
    if self.contents.text_size(text).width > rect.width # Check if this text is too long for the window
      @text_too_long = true if index == self.index
      split_chars = []
      x = chars[0][1]
      ...
      text.chars.each_with_index{|char, i| 
        next if i < @text_shift && index == self.index  # skip letter for the running text
        break if  total_width > real_width
        edited_chars = chars[i].clone
        edited_chars[1] = x
        x += edited_chars[3]
        total_width += edited_chars[3]
        split_chars.push(edited_chars)
        # if this is the last letter, set as the last phase
        if i == text.chars.length - 1 && @text_shift > 0 && index == self.index
          @text_move_timer = System.uptime
          @text_phase = 2
