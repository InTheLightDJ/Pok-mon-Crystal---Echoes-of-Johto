...
module MessageConfig
  LIGHT_TEXT_MAIN_COLOR    = Color.white
  LIGHT_TEXT_SHADOW_COLOR  = Color.black
  DARK_TEXT_MAIN_COLOR     = Color.black
  DARK_TEXT_SHADOW_COLOR   = Color.white
  MALE_TEXT_MAIN_COLOR     = Color.blue  # Used by message tag "\b"
  MALE_TEXT_SHADOW_COLOR   = DARK_TEXT_SHADOW_COLOR
  FEMALE_TEXT_MAIN_COLOR   = Color.red   # Used by message tag "\r"
  FEMALE_TEXT_SHADOW_COLOR = DARK_TEXT_SHADOW_COLOR
  # Default font
  FONT_NAME                = "Gen II"
  FONT_SIZE                = 36
  FONT_Y_OFFSET            = 8
  # Small font
  SMALL_FONT_NAME         = "Gen II Small"
  SMALL_FONT_SIZE         = FONT_SIZE
  SMALL_FONT_Y_OFFSET     = FONT_Y_OFFSET
  # Narrow font
  NARROW_FONT_NAME        = "Gen II Narrow"
  NARROW_FONT_SIZE        = FONT_SIZE
  NARROW_FONT_Y_OFFSET    = FONT_Y_OFFSET
  # Bold font
  BOLD_FONT_NAME          = "Gen II Bold"
  BOLD_FONT_SIZE          = FONT_SIZE
  BOLD_FONT_Y_OFFSET      = FONT_Y_OFFSET
  # Italic font
  ITALIC_FONT_NAME        = "Gen II Italic"
  ITALIC_FONT_SIZE        = FONT_SIZE
  ITALIC_FONT_Y_OFFSET    = FONT_Y_OFFSET
  ...
  def self.pbDefaultBoldFontName
    return MessageConfig.pbTryFonts(BOLD_FONT_NAME)
  end

  def self.pbDefaultItalicFontName
    return MessageConfig.pbTryFonts(ITALIC_FONT_NAME)
  end
  ...
  def self.pbGetBoldFontName
    @@boldFont = pbDefaultBoldFontName if !@@boldFont
    return @@boldFont
  end

  def self.pbGetItalicFontName
    @@italicFont = pbDefaultItalicFontName if !@@italicFont
    return @@italicFont
  end
  ...
  def self.pbSetBoldFontName(value)
    @@boldFont = MessageConfig.pbTryFonts(value)
    @@boldFont = MessageConfig.pbDefaultBoldFontName if @@boldFont == ""
  end

  def self.pbSetItalicFontName(value)
    @@italicFont = MessageConfig.pbTryFonts(value)
    @@italicFont = MessageConfig.pbDefaultItalicFontName if @@italicFont == ""
  end
...
def pbPositionNearMsgWindow(cmdwindow, msgwindow, side)
  ...
  if msgwindow
    ...
    when :right
      cmdwindow.x = msgwindow.x + msgwindow.width - cmdwindow.width + 25
    else
...
def pbSetBoldFont(bitmap)
  bitmap.font.name = MessageConfig.pbGetBoldFontName
  bitmap.font.size = MessageConfig::BOLD_FONT_SIZE
  bitmap.text_offset_y = MessageConfig::BOLD_FONT_Y_OFFSET
end

# Sets a bitmap's font to the system italic font.
def pbSetItalicFont(bitmap)
  bitmap.font.name = MessageConfig.pbGetItalicFontName
  bitmap.font.size = MessageConfig::ITALIC_FONT_SIZE
  bitmap.text_offset_y = MessageConfig::ITALIC_FONT_Y_OFFSET
end
...
# Method used to make Tileset-swap system work (always before fading).
# Only works if TIME_SHADING = false.
def refresh_owmap
  if !Settings::TIME_SHADING && $scene.is_a?(Scene_Map) &&
    !($scene.is_a?(Scene_DebugIntro))
    $game_map.need_refresh = true
    $game_map.pbChangeTileset
  end
end
