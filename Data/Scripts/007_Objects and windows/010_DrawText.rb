...
#  end
  ...
  while position <= textchars.length
    ...
    if boldcount > 0
      fontname = MessageConfig.pbGetBoldFontName
    elsif italiccount > 0
      fontname = MessageConfig.pbGetItalicFontName
    else
      fontname = defaultfontname
    end
    if graphic
  ...
  while (c = textmsg.slice!(/\n|[^ \r\t\f\n\-]*\-+|(\S*([ \r\t\f]?))/)) != nil
    ...
    words.length.times do |i|
      ...
      if word && word != ""
        ...
        ret.push([word, x, y, textwidth, lineheight, color])
        x += textwidth
        dims[0] = x if dims && dims[0] < x
      end
