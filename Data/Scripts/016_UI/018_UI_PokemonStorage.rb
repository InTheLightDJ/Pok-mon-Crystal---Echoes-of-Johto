...
class PokemonBoxArrow < Sprite
  ...
  def initialize(viewport = nil)
    ...
    if !$player.female?
      ...
      @handsprite.addBitmap("point1", "Graphics/UI/Storage/cursor_point_1_f")
      @handsprite.addBitmap("point2", "Graphics/UI/Storage/cursor_point_2_f")
      @handsprite.addBitmap("grab", "Graphics/UI/Storage/cursor_grab_f")
      @handsprite.addBitmap("fist", "Graphics/UI/Storage/cursor_fist_f")
      @handsprite.addBitmap("point1q", "Graphics/UI/Storage/cursor_point_1_q_f")
      @handsprite.addBitmap("point2q", "Graphics/UI/Storage/cursor_point_2_q_f")
      @handsprite.addBitmap("grabq", "Graphics/UI/Storage/cursor_grab_q_f")
      @handsprite.addBitmap("fistq", "Graphics/UI/Storage/cursor_fist_q_f")
    end
  ...
  def x=(value)
    ...
    heldPokemon.x = self.x - 6 if holding?
  end
  ...
  def y=(value)
    ...
    heldPokemon.y = self.y + 8 if holding?
  end
...
class PokemonBoxSprite < Sprite
  ...
  def refresh
    if @refreshBox
      ...
      getBoxBitmap
      @contents.clear
      @contents.blt(0, 0, @boxbitmap.bitmap, Rect.new(0, 0, Graphics.width, Graphics.height))
      @boxarrowbitmap&.dispose
      @boxarrowbitmap = AnimatedBitmap.new("Graphics/UI/Storage/box_arrow")
      @contents.blt(120, 76, @boxarrowbitmap.bitmap, Rect.new(0, 0, Graphics.width, Graphics.height))
      pbSetSystemFont(@contents)
      widthval = @contents.text_size(boxname).width
      xval = 122
      pbDrawShadowText(@contents, xval, 80, 188, 26, boxname, Color.new(0, 0, 0), nil, 1)
      @refreshBox = false
    end
    yval = self.y + 114
...
class PokemonBoxPartySprite < Sprite
  ...
  def initialize(party, viewport = nil)
    super(viewport)
    @party = party
    @boxbitmap = AnimatedBitmap.new("Graphics/UI/Storage/box_0_party")
    @pokemonsprites = []
  ...
  def background=(value)
    return if @background && @background == value
    @background = value
    @boxbitmap&.dispose
    @boxbitmap = AnimatedBitmap.new("Graphics/UI/Storage/box_#{@background}_party")
    refresh
  end
  ...
  def refresh
    ...
    yvalues = []
    Settings::MAX_PARTY_SIZE.times do |i|
      xvalues.push(10 + (32 * (i % 3)))
      yvalues.push(22 + (32 * (i / 3)))
    end
...
class PokemonStorageScene
  ...
  def pbStartBox(screen, command)
    ...
    @command = command
    addBackgroundPlane(@sprites, "background", "bg_white_general", @bgviewport)
    @sprites["box"] = PokemonBoxSprite.new(@storage, @storage.currentBox, @boxviewport)
    @sprites["overlay"] = BitmapSprite.new(Graphics.width, Graphics.height, @boxsidesviewport)
    pbSetSystemFont(@sprites["overlay"].bitmap)
    @sprites["overlaysmall"] = BitmapSprite.new(Graphics.width, Graphics.height, @boxsidesviewport)
    ...
    @sprites["pokemon"].y = 56
    @sprites["boxparty"] = PokemonBoxPartySprite.new(@storage.party, @boxsidesviewport)
    if command != 2   # Drop down tab only on Deposit
      @sprites["boxparty"].x = 0
    end
    @sprites["boxparty"].y = command != 2 ? Graphics.height : Graphics.height - @sprites["boxparty"].height
    @markingbitmap = AnimatedBitmap.new("Graphics/UI/Summary/markings")
    @sprites["markingbg"] = IconSprite.new(Graphics.width / 2 - 8, Graphics.height - 196, @boxsidesviewport)
  ...
  def pbSetArrow(arrow, selection)
    ...
    when -3   # Close Box
      ...
      arrow.x = (126 + (32 * (selection % PokemonBox::BOX_WIDTH)))
      arrow.y = (96 + (32 * (selection / PokemonBox::BOX_WIDTH)))
    end
  end
  ...
  def pbChangeSelection(key, selection)
    ...
    when Input::UP
      case selection
      when -1   # Box name
        selection = PokemonBox::BOX_SIZE - (PokemonBox::BOX_WIDTH * 2 / 3)   # 25
      when -2   # Party
        selection = -3
      when -3   # Close Box
        selection = -2 #PokemonBox::BOX_SIZE - (PokemonBox::BOX_WIDTH / 3)   # 28
      else
    ...
    when Input::DOWN
      ...
      when -3   # Close Box
        ...
        selection += PokemonBox::BOX_WIDTH
        selection = -1 if selection >= PokemonBox::BOX_SIZE # Box name
      end
    when Input::LEFT
      if selection == -1   # Box name
        selection = -4   # Move to previous box
      elsif selection == [-2, -3] # Party, Close Box
        selection += PokemonBox::BOX_WIDTH
      elsif (selection % PokemonBox::BOX_WIDTH) == 0 && (selection / PokemonBox::BOX_WIDTH) == 4 # on the fifth row
        selection = -3   # Move to close box
      elsif (selection % PokemonBox::BOX_WIDTH) == 0
        selection = -2   # Move to party
      else
        selection -= 1
      end
    when Input::RIGHT
      if selection == -1    # Box name
        ...
        selection = PokemonBox::BOX_WIDTH * 3 # Move to the forth row
      elsif selection == -3 # Close Box
        selection = PokemonBox::BOX_WIDTH * 4 # Move to the fifth row
  ...
  def pbPartySetArrow(arrow, selection)
    ...
    ybox = Graphics.height - @sprites["boxparty"].height
    xvalues = []
    yvalues = []
    Settings::MAX_PARTY_SIZE.times do |i|
      xvalues.push(14 + (32 * (i % 3)))
      yvalues.push(190 + (32 * (i / 3)))
    end
    xvalues.push(46)
    yvalues.push(254)
    arrow.angle = 0
  ...
  def pbPartyChangeSelection(key, selection)
    ...
    when Input::UP
      if selection == Settings::MAX_PARTY_SIZE
        selection = Settings::MAX_PARTY_SIZE - 2
      else
        selection -= 3
        selection = Settings::MAX_PARTY_SIZE if selection < 0
      end
  ...
  def pbChangeBackground(wp)
    ...
    Graphics.update
    self.update
    # Fade old background to black
    @sprites["box"].color = Color.new(0, 0, 0, 255)
    ...
    @sprites["box"].color = Color.new(0, 0, 0, 0)
  ...
  def pbShowPartyTab
    ...
    # Change party box background
    @sprites["boxparty"].background = @storage[@storage.currentBox].background
    @sprites["boxparty"].y = Graphics.height - @sprites["boxparty"].height
    Input.update
  ...
  def pbBoxName(helptext, minchars, maxchars)
    ...
    ret = pbEnterBoxName(helptext, minchars, maxchars, @storage[@storage.currentBox].name)
  ...
  def pbMarkingSetArrow(arrow, selection)
    if selection >= 0
      xvalues = [178, 226, 274, 178, 226, 274, 226, 226]
      yvalues = [92, 92, 92, 136, 136, 136, 180, 224]
      arrow.angle = 0
      arrow.mirror = false
      arrow.ox = 0
      arrow.oy = 0
  ...
  def pbMark(selected, heldpoke)
    ...
    msg = _INTL("Mark your κΜ.")
    msgwindow = Window_UnformattedTextPokemon.newWithSize("", 0, 0, Graphics.width / 2 - 8, 32)
    msgwindow.viewport       = @viewport
    msgwindow.visible        = true
    msgwindow.letterbyletter = false
    msgwindow.text           = msg
    msgwindow.resizeHeightToFit(msg, Graphics.width / 2 - 8)
    pbBottomLeft(msgwindow)
    ...
    loop do
      ...
      if redraw
        ...
        (@markingbitmap.bitmap.width / MARK_WIDTH).times do |i|
          ...
          @sprites["markingoverlay"].bitmap.blt(181 + (48 * (i % 3)), 115 + (44 * (i / 3)),
                                                @markingbitmap.bitmap, markrect)
        end
        textpos = [
          [_INTL("OK"), Graphics.width / 2 + 78, 204, :center, base, shadow],
          [_INTL("Cancel"), Graphics.width / 2 + 78, 248, :center, base, shadow]
        ]
  ...
  def pbUpdateOverlay(selection, party = nil)
    ...
    overlay.clear
    # Overlay used for small font text
    overlay_small = @sprites["overlaysmall"].bitmap
    overlay_small.clear
    # Color variables
    black_base   = Color.new(0, 0, 0)
    white_base   = Color.new(248, 248, 248)
    black_shadow = black_base
    white_shadow = white_base
    # Party and Exit texts (left side)
    pbDrawTextPositions(
      overlay_small,
      [[_INTL("Markings"), 20, 173, :left, black_base, white_base, :outline],
       [_INTL("Party: {1}", (@storage.party.length rescue 0)), 22, 220, :left, white_base],
       [_INTL("Exit"), 40, 252, :left, white_base]]
    )
    pokemon = nil
    if @screen.pbHeldPokemon
    ...
    pokespecies = pokemon.speciesName
    textstrings = [
      [pokename, 116, 0, :left, white_base],
      ["/" + pokespecies, 116, 16, :left, white_base]
    ]
    small_text = []
    if !pokemon.egg?
      imagepos = []
      # Pokémon's gender
      if pokemon.male?
        textstrings.push([_INTL("♂"), 66, 132, :left, white_base])
      elsif pokemon.female?
        textstrings.push([_INTL("♀"), 66, 132, :left, white_base])
      end
      # Pokémon's level
      textstrings.push([_INTL("Λ{1}", pokemon.level.to_s), 2, 132, :left, white_base])
      # Pokémon's ability
      if pokemon.ability
        small_text.push([_INTL("Ab./{1}", pokemon.ability.name), 116, 32, :left, white_base])
      else
        small_text.push([_INTL("No ability"), 116, 32, :left, white_base])
      end
      # Pokémon's item
      if pokemon.item
        small_text.push([pokemon.item.name, 136, 50, :left, white_base])
      else
        small_text.push([_INTL("No item"), 136, 50, :left, white_base])
      end
      # Item and shiny icons
      imagepos.push(["Graphics/UI/Party/icon_item", 116, 50])
      imagepos.push(["Graphics/UI/shiny", 88, 132]) if pokemon.shiny?
      # Type icons
      typebitmap = AnimatedBitmap.new(_INTL("Graphics/UI/types"))
      pokemon.types.each_with_index do |type, i|
        type_number = GameData::Type.get(type).icon_position
        type_rect = Rect.new(0, type_number * GameData::Type::ICON_SIZE[1], *GameData::Type::ICON_SIZE)
        type_x = (pokemon.types.length == 1) ? 2 : 2 + (GameData::Type::ICON_SIZE[0] * i)
        overlay.blt(type_x, 114, typebitmap.bitmap, type_rect)
      end
      typebitmap.dispose
      # Markins
      drawMarkings(overlay, 9, 155, 100, 32, pokemon.markings)
      pbDrawImagePositions(overlay, imagepos)
    end
    pbDrawTextPositions(overlay, textstrings)
    pbDrawTextPositions(overlay_small, small_text)
...
class PokemonStorageScreen
  ...
  def pbRelease(selected, heldpoke)
    ...
    if command == 1
      ...
      # If releasing a caught roamer, reset it so it roams again
      Settings::ROAMING_SPECIES.each_with_index do |data, i|
        next unless pokemon.isSpecies?(data[0])
        if $PokemonGlobal.roamPokemonCaught[i]
          # Was previously caught through the normal roamer encounter system
          $PokemonGlobal.roamPokemon[i]       = nil
          $PokemonGlobal.roamPokemonCaught[i] = false
          $PokemonGlobal.roamPosition[i]      = nil
          pbDisplay(_INTL("{1} has returned to the wild!", pkmnname))
          break
        elsif data[0] == :SUICUNE && !$game_switches[94]
          # Suicune was caught via its stationary story encounter (not as a roamer).
          # Releasing it for the first time activates it as a roamer.
  ...
  def pbBoxCommands
    ...
    when 1
      ...
      wpaper = pbShowCommands(_INTL("Pick the theme."), papers[0], index)
