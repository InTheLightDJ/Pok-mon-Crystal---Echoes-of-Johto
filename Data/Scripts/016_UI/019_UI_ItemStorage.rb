...
class Window_PokemonItemStorage < Window_DrawableCommand
  ...
  def drawItem(index, _count, rect)
    ...
    if index == @bag.length
      ...
      baseColor = (index == @sortIndex) ? Color.red : self.baseColor
      ...
      if GameData::Item.get(item).show_quantity?
        ...
        textpos.push([qty, xQty, rect.y + 16, :left, baseColor, self.shadowColor])
      end
    end
...
class ItemStorage_Scene
  ...
  def pbStartScene(bag)
    ...
    @sprites["icon"] = ItemIconSprite.new(159, 176, nil, @viewport)
    # Item list
    @sprites["itemwindow"] = Window_PokemonItemStorage.new(@bag, 0, 14, 317, (ITEMSVISIBLE * 41))
    @sprites["itemwindow"].viewport    = @viewport
    @sprites["itemwindow"].index       = 0
    @sprites["itemwindow"].baseColor   = BASECOLOR
    @sprites["itemwindow"].shadowColor = SHADOWCOLOR
    @sprites["itemwindow"].refresh
    # Title
    @sprites["pocketwindow"] = BitmapSprite.new(Graphics.width, 32, @viewport)
    @sprites["pocketwindow"].x = 92
    @sprites["pocketwindow"].y = 16
    pbSetNarrowFont(@sprites["pocketwindow"].bitmap)
    # Item description
    @sprites["itemtextwindow"] = Window_UnformattedTextPokemon.newWithSize("", 0, 190, Graphics.width + 4, 96, @viewport)
    @sprites["itemtextwindow"].baseColor   = BASECOLOR
    @sprites["itemtextwindow"].shadowColor = SHADOWCOLOR
  ...
  def pbRefresh
    ...
    drawTextEx(bm, 0, 0, bm.width, 1, @title, BASECOLOR, SHADOWCOLOR)
