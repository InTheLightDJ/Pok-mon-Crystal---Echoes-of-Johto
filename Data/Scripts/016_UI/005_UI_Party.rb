...
class PokemonPartyConfirmCancelSprite < Sprite
  ...
  def initialize(text, x, y, narrowbox = false, viewport = nil)
    ...
    textpos = [[text, 16, 0, :left, Color.new(0, 0, 0), Color.new(248, 248, 248)]]
...
class PokemonPartyCancelSprite2 < PokemonPartyConfirmCancelSprite
  def initialize(viewport = nil)
    super(_INTL("Cancel"), 200, 208, true, viewport)
  end
end
...
class Window_CommandPokemonColor < Window_CommandPokemon
  ...
  def drawItem(index, _count, rect)
    ...
    real_width = rect.width - 16
    if self.contents.text_size(text).width > rect.width # Check if this text is too long for the window
...
class PokemonPartyBlankPanel < Sprite
  ...
  def initialize(_pokemon, index, viewport = nil)
    ...
    @panelbgsprite = AnimatedBitmap.new("Graphics/UI/Party/panel")
...
class PokemonPartyPanel < Sprite
  ...
  def initialize(pokemon, index, viewport = nil)
    ...
    @panelbgsprite.addBitmap("panel", "Graphics/UI/Party/panel")
    @panelbgsprite.addBitmap("panelsel", "Graphics/UI/Party/panel_sel")
    ...
    @hpbgsprite.addBitmap("hp_bg", "Graphics/UI/Party/overlay_hp_back")
  ...
  def visible=(value)
    super
    @panelbgsprite.visible = value
    @hpbgsprite.z = self.z + (value ? 1 : -1)
    @pkmnsprite.visible = value
    @helditemsprite.visible = value
  ...
  def refresh_panel_graphic
    ...
    if self.selected
      if self.preselected
        ...
        @panelbgsprite.changeBitmap("panelsel")
      end
    else
      if self.preselected
        @panelbgsprite.changeBitmap("swap")
      else
        @panelbgsprite.changeBitmap("panel")
      end
    end
  ...
  def draw_name
    pbDrawTextPositions(@overlaysprite.bitmap,
                        [[@pokemon.name, 48, 16, :left, TEXT_BASE_COLOR, TEXT_SHADOW_COLOR]])
  end
  ...
  def draw_level
    ...
    if @pokemon.level != 100
      pbDrawTextPositions(@overlaysprite.bitmap,
        [[_INTL("Λ{1}", @pokemon.level.to_s), 151, 32, :center, TEXT_BASE_COLOR, TEXT_SHADOW_COLOR]])
    else
      pbDrawTextPositions(@overlaysprite.bitmap,
        [[@pokemon.level.to_s, 151, 32, :center, TEXT_BASE_COLOR, TEXT_SHADOW_COLOR]])
    end
    pbSetSystemFont(@overlaysprite.bitmap)
  end
  ...
  def draw_gender
    ...
    base_color   = (@pokemon.male?) ? Color.blue : Color.red
    pbDrawTextPositions(@overlaysprite.bitmap,
                        [[gender_text, 48, 32, :left, base_color, TEXT_SHADOW_COLOR]])
  end
  ...
  def draw_hp
    ...
    pbDrawTextPositions(@overlaysprite.bitmap,
                        [[hp_text, 320, 16, :right, TEXT_BASE_COLOR, TEXT_SHADOW_COLOR]])
    # HP bar
    if @pokemon.able?
  ...
  def draw_status
    ...
    @overlaysprite.bitmap.blt(84, 32, @statuses.bitmap, statusrect)
  end

  def draw_shiny_icon
    return if @pokemon.egg? || !@pokemon.shiny?
    pbDrawImagePositions(@overlaysprite.bitmap,
                         [["Graphics/UI/shiny", 64, 31, 0, 0, 16, 16]])
  end

  def draw_annotation
    return if !@text || @text.length == 0
    pbDrawTextPositions(@overlaysprite.bitmap,
                        [[@text, 190, 24, :left, TEXT_BASE_COLOR, TEXT_SHADOW_COLOR]])
  end
...
class PokemonParty_Scene
  def pbStartScene(party, starthelptext, annotations = nil, multiselect = false, can_access_storage = false)
    ...
    addBackgroundPlane(@sprites, "partybg", "bg_white_general", @viewport)
    ...
    @sprites["storagetext"].x           = 92
    @sprites["storagetext"].y           = Graphics.height - @sprites["messagebox"].height - 6
    @sprites["storagetext"].z           = 10
    @sprites["storagetext"].viewport    = @viewport
    @sprites["storagetext"].baseColor   = Color.black
    @sprites["storagetext"].shadowColor = Color.white
  ...
  def pbShowCommands(helptext, commands, index = 0)
    ...
    using(cmdwindow = Window_CommandPokemonColor.new(commands)) do
      ...
      cmdwindow.width = [cmdwindow.width, Graphics.width].min
      pbBottomRight(cmdwindow)
  ...
  def pbSwitchBegin(oldid, newid)
    ...
    @sprites["pokemon#{oldid}"].visible = false
    @sprites["pokemon#{newid}"].visible = false
    timer_start = System.uptime
    loop do
      Graphics.update
      Input.update
      break if System.uptime - timer_start >= 0.15
    end
  end
...
class PokemonPartyScreen
  ...
  def pbPokemonGiveScreen(item)
    @scene.pbStartScene(@party, _INTL("To which κΜ?"))
  ...
  def pbPokemonMultipleEntryScreenEx(ruleset)
    annot = []
    statuses = []
    ordinals = [_INTL("CAN NOT!"), _INTL("NO ENTER"), _INTL("BANNED")]
