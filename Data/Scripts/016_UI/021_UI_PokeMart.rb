...
class Window_PokemonMart < Window_DrawableCommand
  ...
  def drawItem(index, count, rect)
    ...
    ypos = rect.y
    if index == count - 1
      textpos.push([_INTL("Cancel"), rect.x, ypos + 4, :left, self.baseColor, self.shadowColor])
    else
      item = @stock[index]
      itemname = @adapter.getDisplayName(item)
      qty = @adapter.getDisplayPrice(item)
      sizeQty = self.contents.text_size(qty).width
      xQty = rect.x + rect.width - sizeQty - 50
      textpos.push([itemname, rect.x, ypos + 4, :left, self.baseColor, self.shadowColor])
      textpos.push([qty, xQty, ypos + 20, :left, self.baseColor, self.shadowColor])
    end
...
class PokemonMart_Scene
  ...
  def pbRefresh
    if @subscene
      ...
      @sprites["qtywindow"].text    = _INTL("Bag<r>x {1}", @adapter.getQuantity(itemwindow.item))
      @sprites["qtywindow"].y       = 0
      itemwindow.refresh
    end
    @sprites["moneywindow"].text = _INTL("<r>{1}", @adapter.getMoneyString)
  end
  ...
  def pbStartBuyOrSellScene(buying, stock, adapter)
    ...
    if $player.female?
      @sprites["background"].setBitmap("Graphics/UI/Mart/bg_f")
    else
      @sprites["background"].setBitmap("Graphics/UI/Mart/bg")
    end
    @sprites["icon"] = ItemIconSprite.new(40, 96, nil, @viewport)
    winAdapter = buying ? BuyAdapter.new(adapter) : SellAdapter.new(adapter)
    @sprites["itemwindow"] = Window_PokemonMart.new(
      stock, winAdapter, 66, 34, Graphics.width, Graphics.height - 118
    )
    ...
    @sprites["itemtextwindow"].shadowColor = Color.white
    ...
    @sprites["moneywindow"].x = Graphics.width / 2
    @sprites["moneywindow"].y = 0
    @sprites["moneywindow"].width = Graphics.width / 2
    @sprites["moneywindow"].height = 54
    @sprites["moneywindow"].baseColor = Color.new(0, 0, 0)
    @sprites["moneywindow"].shadowColor = Color.new(248, 248, 248)
    ...
    @sprites["qtywindow"].width = Graphics.width / 2
    @sprites["qtywindow"].height = 54
    @sprites["qtywindow"].baseColor = Color.new(0, 0, 0)
    @sprites["qtywindow"].shadowColor = Color.new(248, 248, 248)
    @sprites["qtywindow"].text = _INTL("Bag<r>x {1}", @adapter.getQuantity(@sprites["itemwindow"].item))
    @sprites["qtywindow"].y    = 0
  ...
  def pbChooseNumber(helptext, item, maximum)
    ...
    using(numwindow = Window_AdvancedTextPokemon.new("")) do   # Showing number of items
      ...
      numwindow.shadowColor = Color.new(248, 248, 248)
  ...
  def pbBuyScreen
    ...
    loop do
      ...
      if GameData::Item.get(item).is_important?
        next if !pbConfirm(_INTL("You want {1}. That will be ${2}.",
        ...
        quantity = @scene.pbChooseNumber(
          _INTL("How many {1}?", itemnameplural), item, maxafford
        )
        next if quantity == 0
        price *= quantity
        if quantity > 1
          next if !pbConfirm(_INTL("You want {1} {2}. They'll be ${3}.",
                                   quantity, itemnameplural, price.to_s_formatted))
        elsif quantity > 0
          next if !pbConfirm(_INTL("You want {1} {2}. That will be ${3}.",
  ...
  def pbSellScreen
    ...
    loop do
      ...
      if !@adapter.canSell?(item)
        pbDisplayPaused(_INTL("{1}? Oh, no. I can't buy that.", itemnameplural))
        next
      end
      ...
      if pbConfirm(_INTL("I can pay ${1}.\nWould that be OK?", price.to_s_formatted))
        ...
        pbDisplayPaused(_INTL("Got ${1} for {2}.",
                               price.to_s_formatted, sold_item_name)) { pbSEPlay("Mart buy item") }
        @scene.pbRefresh
      end
      @scene.pbHideMoney
    end
...
def pbPokemonMart(stock, speech = nil, cantsell = false)
  ...
  loop do
    ...
    cmd = pbMessage(_INTL("Can I do anything else for you?"), commands, cmdQuit + 1)
  end
