...
class Window_PokemonBag < Window_DrawableCommand
  ...
  def drawCursor(index, rect)
    if self.index == index
      ...
      pbCopyBitmap(self.contents, bmp, rect.x + 3, rect.y + 28)
    end
  end
  ...
  def drawItem(index, _count, rect)
    ...
    if index == self.itemCount - 1
      textpos.push([_INTL("Cancel"), rect.x, ypos, :left, self.baseColor, self.shadowColor])
    else
      item = (@filterlist) ? thispocket[@filterlist[@pocket][index]][0] : thispocket[index][0]
      baseColor   = self.baseColor
      shadowColor = self.shadowColor
      if @sorting && index == self.index
        $player.female? ? baseColor = Color.red : Color.blue
        shadowColor = Color.white
      end
      textpos.push(
        [@adapter.getDisplayName(item), rect.x, ypos, :left, baseColor, shadowColor]
      )
      ...
      if item_data.is_important?
        if @bag.registered?(item)
          pbDrawImagePositions(
            self.contents,
            [[_INTL("Graphics/UI/Bag/icon_register"), rect.x + rect.width - 60, rect.y + 32 + 56, 0, 14, -1, 14]]
          )
          showing_register_icon = true
        elsif pbCanRegisterItem?(item)
          pbDrawImagePositions(
            self.contents,
            [[_INTL("Graphics/UI/Bag/icon_register"), rect.x + rect.width - 60, rect.y + 32 + 56, 0, 0, -1, 14]]
          )
          showing_register_icon = true
        end
      end
      if item_data.show_quantity? && !showing_register_icon
        ...
        textpos.push([qtytext, xQty, ypos_qty, :left, baseColor, shadowColor])
      end
    end
...
class PokemonBag_Scene
  ...
  ITEMTEXTBASECOLOR     = ITEMLISTBASECOLOR
  ...
  def pbStartScene(bag, choosing = false, filterproc = nil, resetpocket = true)
    ...
    @sprites["itemlist"] = Window_PokemonBag.new(@bag, @filterlist, lastpocket, 64, -12, 280, 64 + (ITEMSVISIBLE * 48))
    ...
    @sprites["itemicon"] = ItemIconSprite.new(40, 152, nil, @viewport)
    @sprites["itemtext"] = Window_UnformattedTextPokemon.newWithSize(
      "", 0, 192, Graphics.width + 4, 96, @viewport
    )
  ...
  def pbRefresh
    ...
    if $player.female?
      @sprites["background"].setBitmap(sprintf("Graphics/UI/Bag/bg_%d_f", @bag.last_viewed_pocket))
    else
        ...
        if itemwindow.sorting
          ...
          if Input.trigger?(Input::LEFT)
            ...
            if itemwindow.pocket != newpocket
              ...
              thispocket = @bag.pockets[itemwindow.pocket]
              pbSEPlay("Bag_ChangePocket", 100, 100)
              pbRefresh
            end
  ...
  def pbStartScreen
    ...
    loop do
      ...
      if cmdRead >= 0 && command == cmdRead   # Read mail
        ...
        command = 0
        loop do
          command = @scene.pbShowCommands([_INTL("Change quantity"),
