...
class Window_PhoneList < Window_CommandPokemon
  ...
  def drawCursor(index, rect)
    if self.index == index
      selarrow = AnimatedBitmap.new("Graphics/UI/Pokegear/phoneSel")
      pbCopyBitmap(self.contents, selarrow.bitmap, rect.x, rect.y + 2)
    end
    return Rect.new(rect.x + 28, rect.y + 8 + 6, rect.width - 16, rect.height)
  end

  def drawItem(index, count, rect)
    return if index >= self.top_row + self.page_item_max
    text = @commands[index].split(":")
    name = @commands[index] == "" ? "----------" : "#{text[0]} :"
    if self.index == index && @switching
      rect = drawCursor(index, rect)
      pbDrawShadowText(self.contents, rect.x, rect.y + (self.contents.text_offset_y || 0),
                       rect.width, rect.height, name, Color.new(224, 0, 0), Color.new(224, 144, 144))
      pbDrawShadowText(self.contents, rect.x + 48, rect.y + (self.contents.text_offset_y || 0) + 16,
                       rect.width, rect.height, text[1], Color.new(224, 0, 0), Color.new(224, 144, 144))
    else
      pbSetSystemFont(self.contents) if @starting
      rect = drawCursor(index, rect)
      pbDrawShadowText(self.contents, rect.x, rect.y + (self.contents.text_offset_y || 0),
                       rect.width, rect.height, name, self.baseColor, self.shadowColor)
      pbDrawShadowText(self.contents, rect.x + 48, rect.y + (self.contents.text_offset_y || 0) + 16,
                       rect.width, rect.height, text[1], self.baseColor, self.shadowColor)
    end
