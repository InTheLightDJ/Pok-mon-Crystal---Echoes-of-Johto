...
class PokemonPokedexInfo_Scene
  ...
  def pbUpdate
    if @page == 2
      ...
      intensity = (intensity_time >= 0.5) ? 248 : 0
  ...
  def pbUpdateDummyPokemon
    ...
    @form = 0 if @form == true   # Fix bad data
    @shiny = false
    metrics_data = GameData::SpeciesMetrics.get_species_form(@species, @form)
    @sprites["infosprite"].setSpeciesBitmap(@species, @gender, @shiny, false, @form)
    @sprites["formfront"]&.setSpeciesBitmap(@species, @gender, @shiny, false, @form)
    if @sprites["formback"]
      @sprites["formback"].setSpeciesBitmap(@species, @gender, @shiny, true, @form, false)
      @sprites["formback"].y = 166
      @sprites["formback"].y += metrics_data.back_sprite[1] * 2
    end
    @sprites["formicon"]&.pbSetParams(@species, @gender, @shiny, @form)
  end
  ...
  def drawPageInfo
    ...
    @selbitmap = AnimatedBitmap.new("Graphics/UI/Pokedex/white_cursor")
    overlay = @sprites["overlay"].bitmap
    ...
    textpos = [
      [_INTL("{1}", species_data.name), 134, 48, :left, base, shadow],
      [_INTL("HT"), 134, 112, :left, base, shadow],
      [_INTL("WT"), 134, 144, :left, base, shadow],
      [_INTL("{1}", indexText), 56, 128, :left, base, shadow],
      [_INTL("+ Desc: C"), 6, 148, :left, base, shadow]
    ]
    if $player.owned?(@species)
      # Write the category
      textpos.push([_INTL("{1} κΜ", species_data.category), 134, 80, :left, base, shadow])
      ...
      if !@show_battled_count
        ...
        if System.user_language[3..4] == "US"   # If the user is in the United States
          ...
          textpos.push([_ISPRINTF("{1:d}'{2:02d}\"", inches / 12, inches % 12), 310, 112, :right, base, shadow])
          textpos.push([_ISPRINTF("{1:4.1f} lbs.", pounds / 10.0), 310, 144, :right, base, shadow])
        else
          textpos.push([_ISPRINTF("{1:.1f} m", height / 10.0), 310, 112, :right, base, shadow])
          textpos.push([_ISPRINTF("{1:.1f} kg", weight / 10.0), 310, 144, :right, base, shadow])
        end
      end
      # Draw the Pokédex entry text
      entry = species_data.pokedex_entry if !entry || entry == ""
      # Get the text divided into pages that will fit on screen
      allPages = Array.new
      normtext = getLineBrokenChunks(overlay, entry, Graphics.width - 8, nil, true)
      ...
      if @entrypage < allPages.length - 1
        overlay.blt(152, 255, @selbitmap.bitmap, Rect.new(0, 0, 16, 16))
      end
      drawTextEx(overlay, 6, 176, Graphics.width - 8, 3,   # overlay, x, y, width, num lines
                 allPages[@entrypage], base, shadow)
      # Draw the footprint
      ...
      # Show the owned icon
      imagepos.push(["Graphics/UI/Pokedex/icon_own", 6, 130])
      # Draw the type icon(s)
      species_data.types.each_with_index do |type, i|
        type_number = GameData::Type.get(type).icon_position
        type_rect = Rect.new(0, type_number * 14, 54, 14)
        overlay.blt(134 + (70 * i), 24, @typebitmap.bitmap, type_rect)
      end
    else
      # Write the category
      textpos.push([_INTL("?????"), 134, 80, :left, base, shadow])
      # Write the height and weight
      if !@show_battled_count
        if System.user_language[3..4] == "US"   # If the user is in the United States
          textpos.push([_INTL("???'??\""), 310, 112, :right, base, shadow])
          textpos.push([_INTL("????.? lbs."), 310, 144, :right, base, shadow])
        else
          textpos.push([_INTL("????.? m"), 310, 112, :right, base, shadow])
          textpos.push([_INTL("????.? kg"), 310, 144, :right, base, shadow])
        end
      end
    end
    # Draw all text
  ...
  def drawPageArea
    ...
    points.length.times do |j|
      ...
      imagepos.push([_INTL("Graphics/UI/Pokedex/icon_nest"), x, y])
    end
    ...
    if points.length == 0
      pbDrawImagePositions(
        overlay,
        [["Graphics/UI/Pokedex/overlay_areanone", 16, 128]]
      )
      textpos.push([_INTL("Area unknown"), Graphics.width / 2, (Graphics.height / 2), :center, base, shadow])
    end
    textpos.push([_INTL("{1}'s nest", GameData::Species.get(@species).name),
                  Graphics.width / 2, 3, :center, Color.new(248, 248, 248), base])
    pbDrawTextPositions(overlay, textpos)
  end
  ...
  def drawPageForms
    ...
    textpos = [
      [GameData::Species.get(@species).name, Graphics.width / 2, Graphics.height - 88, :center, base, shadow],
      [formname, Graphics.width / 2, Graphics.height - 56, :center, base, shadow]
    ]
    # Draw all text
    pbDrawTextPositions(overlay, textpos)
  end
