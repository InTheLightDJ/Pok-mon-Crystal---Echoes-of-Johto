...
def pbDrawGauge(bitmap, rect, color, value, maxValue)
  ...
  if rect.width >= 4 && rect.height >= 4
    bitmap.fill_rect(rect.x + 2, rect.y + 2, rect.width - 4, rect.height - 4, Color.black)
...
def calcPoint(x, y, distance, angle)
  ...
  point[0] += x
  point[1] += y
  # Make the coordinates even number
  point[0] = point[0].floor.odd? ? point[0].floor + 1 : point[0].floor
  point[1] = point[1].floor.odd? ? point[1].floor + 1 : point[1].floor
  return point
end
...
class Window_PurifyChamberSets < Window_DrawableCommand
  ...
  def drawItem(index, _count, rect)
    ...
    if index == @switching
      textpos.push([(index + 1).to_s, rect.x, rect.y, :left, Color.red, self.shadowColor])
    else
    ...
    if @chamber.setCount(index) > 0
      pbDrawGauge(self.contents, Rect.new(rect.x + 16, rect.y + 6, 48, 8),
                  Color.new(248, 248, 248), @chamber[index].tempo, PurifyChamber.maximumTempo)
    end
...
class DirectFlowDiagram
  ...
  def ensurePoint(j)
    if !@points[j] || @points[j].disposed?
      ...
      # Stronger the flow, the bigger the dots
      if @strength == 2 # Strong
        @points[j].bitmap.fill_rect(2, 0, 4, 8, Color.black)
        @points[j].bitmap.fill_rect(0, 2, 8, 4, Color.black)
      else
        @points[j].bitmap.fill_rect(2, 2, 4, 4, Color.black)
      end
    end
    @points[j].tone = (@strength == 2) ? Tone.new(232, 232, 248) : Tone.new(88, 152, 248)
  ...
  def update
    ...
    while i < @distance
      ...
      i += (@strength == 2) ? 40 : 60 # Angle between each dots
    end
...
class PurifyChamberSetView < Sprite
  ...
  def initialize(chamber, set, viewport = nil)
    ...
    @info = BitmapSprite.new(Graphics.width, 64, viewport)
    pbSetSmallFont(@info.bitmap)
    @infobox = pbCreateMessageWindow(viewport)
    @infobox.height = 74
    @infobox.width = Graphics.width
    @infobox.y = Graphics.height - @infobox.height
    @infobox.z = @info.z - 1
    @poketype = BitmapSprite.new(Graphics.width, Graphics.height, viewport)
    @flows = []
  ...
  def refresh
    ...
    @view.visible = false
    @info.bitmap.clear
    @poketype.bitmap.clear
    textpos = []
    imgPos=[]
    if pkmn
      pkmn.types.each_with_index do |type, i|
        type_number = GameData::Type.get(type).icon_position
        imgPos.push(["Graphics/UI/types", 14 + 54*i, 34, 0, type_number * 14, 54, 14])
      end
      textpos.push([_INTL("{1}", pkmn.name), 14, 16, :left, Color.black])
      textpos.push([_INTL("Lv.{1}", pkmn.level),304, 16, :right, Color.black])
      # draw heart gauge
      pbDrawGauge(@info.bitmap, Rect.new(14, 50, 120, 10),
                  Color.new(192, 0, 256), pkmn.heart_gauge, pkmn.max_gauge_size)
      # draw flow gauge
      textpos.push([_INTL("Flow"), 232, 46, :right, Color.black])
      pbDrawGauge(@info.bitmap, Rect.new(240, 50, 64, 8),
                  Color.white, @chamber.chamberFlow(@set), 7)
    end
    if @chamber.setCount(@set) > 0
      textpos.push([_INTL("Tempo"), 232, 32, :right, Color.black])
      # draw tempo gauge
      pbDrawGauge(@info.bitmap, Rect.new(240, 36, 64, 8),
                  Color.new(88, 152, 248), @chamber[@set].tempo, PurifyChamber.maximumTempo)
    end
    pbDrawTextPositions(@info.bitmap, textpos)
    pbDrawImagePositions(@info.bitmap, imgPos)
    imgPos = []
    @info.y = Graphics.height - @infobox.height
    ...
    (PurifyChamber::SETSIZE * 2).times do |i|
      ...
      if pkmn
        x = 244 ; y = 12 # no 1 always on top
        if angle == 270 # no 2 for 2 pokemon and no 3 for 4 pokemon
          x = 244 ; y = 172
        elsif i == 2 && angle == 330 # no 2 for 3 pokemon
          x = 262 ; y = 172
        elsif i == 2 && angle == 360 # no 2 for 4 pokemon
          x = 262 ; y = 124
        elsif i == 4 && angle == 210 # no 3 for 3 pokemon
          x = 118 ; y = 172
        elsif i == 6 && angle == 180 # no 4 for 4 pokemon
          x = 118 ; y = 124
        end
        pkmn.types.each_with_index do |type, j|
          type_number = GameData::Type.get(type).icon_position
          imgPos.push(["Graphics/UI/types", x, y + 14 * j, 0, type_number * 14, 54, 14])
        end
      end
      pbDrawImagePositions(@poketype.bitmap, imgPos)
      point = calcPoint(218, 110, 80, angle)
  ...
  def color=(value)
    ...
    @infobox.color = value.clone
    @poketype.color = value.clone
  end
  ...
  def dispose
    ...
    @info.dispose
    @poketype.dispose
    pbDisposeMessageWindow(@infobox)
    @__sprites.clear
    super
  end
end
...
class PurifyChamberScene
  ...
  def pbOpenSet(set)
    ...
    addBackgroundOrColoredPlane(@sprites, "bg", "purifychamber_bg",
                                Color.new(255, 255, 255, 0), @viewport)
    @sprites["setwindow"] = Window_PurifyChamberSets.new(
      @chamber, 0, 0, 112, Graphics.height - 74, @viewport
    )
    ...
    @sprites["overlay"].z = @sprites["setwindow"].z + 1
    pbSetSmallFont(@sprites["overlay"].bitmap)
    imgPos=[["Graphics/UI/purify_chamber_set", 12, 2]]
    pbDrawImagePositions(@sprites["overlay"].bitmap, imgPos)
